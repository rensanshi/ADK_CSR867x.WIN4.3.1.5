...
static void gattAncsInitCfm(const GATT_ANCS_INIT_CFM_T *cfm)
{
    ...
    if((cfm->status == gatt_ancs_status_success) && (ancs != NULL) && sinkGattGetAncsNotificationEnableMask())
    {
        GattAncsSetNSNotificationEnableRequest(ancs, TRUE, sinkGattGetAncsNotificationEnableMask());
    }
}
