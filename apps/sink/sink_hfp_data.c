...
static void sinkHfpUpdateInitParams(hfp_init_params *hfp_init_param, sink_hfp_data_readonly_config_def_t *hfp_config_data)
{
    ...
    hfp_init_param->supported_wbs_codecs = hfp_config_data->hfp_init_params.supported_wbs_codecs_def;
...
RETURNS
    void
*/
void sinkHfpDataSetProfileConnectedStatus(bool value, uint16 index)
{
...
*/
bool sinkHfpDataGetProfileStatusConnected(uint16 index)
{
    return G_HFP_DATA.profile_data[index].status.connected ? TRUE:FALSE;
}
