...
static void bmInitConfigDurations (void);
...
static void bmCheckButtonsAfterReadingConfig( void );
...
{
    sink_button_config_def_t *button_config = NULL;

    if(configManagerGetReadOnlyConfig(SINK_BUTTON_CONFIG_BLK_ID, (const void **)&button_config))
    {
        memcpy(BUTTON_GDATA->button_config, button_config, (sizeof(button_config_type_t)));
...
static void bmInitButtonConfig(void)
{
    ...
    /* Set up the global data from the read configuration data */
    bmInitConfigDurations();
...
static void bmInitConfigDurations (void)
{
    button_config_type_t *pButtons = NULL;

    bmSetupButtonConfig();

    pButtons = BUTTON_GDATA->button_config;

    if ((pButtons->debounce_number == 0 ) || ( pButtons->debounce_period_ms == 0 ) )
    {
        /*use defaults*/
        BM_DEBUG(("BM: DEFAULT button debounce\n")) ;
        pButtons->debounce_number =  BUTTON_PIO_DEBOUNCE_NUM_CHECKS;
        pButtons->debounce_period_ms = BUTTON_PIO_DEBOUNCE_TIME_MS ;
    }
    else
    {
        BM_DEBUG(("BM: Debounce[%x][%x]\n" , pButtons->debounce_number, pButtons->debounce_period_ms));
    }
}
    ...
    {       /*Each Entry*/
        for (lEvIndex = 0 ; lEvIndex < lNumEvents ; lEvIndex ++)
        {
            ...
            if (event_config)
            {
                ...
                lMask = bmGetButtonEventConfigPioMask(event_config);
                ...
                if (lMask == pButtonMask)
                {
                    ...
                    if ((ButtonsTime_t)event_config->type == pDuration)
                    {
    ...
    for (block = 0; block < num_blocks; block++)
    {
        ...
        for(event = 0; event < lNumEvents; event++)
        {
            ...
            if (mask)
            {
                ...
                PioCommonAllbitsOR(&(BUTTON_GDATA->pio_debounce_mask),
