...
{
    if((BdaddrIsSame(getA2dpLinkBdAddr(priority), sinkAvrcpGetLinkBdAddr(a2dp_primary))&&(sinkAvrcpGetPlayStatus(a2dp_primary) == (SinkAvrcp_play_status)play_status))||
       (BdaddrIsSame(getA2dpLinkBdAddr(priority), sinkAvrcpGetLinkBdAddr(a2dp_secondary))&&(sinkAvrcpGetPlayStatus(a2dp_secondary) == (SinkAvrcp_play_status)play_status)))
    {
        return TRUE;
    }
    else
        ...
        if (configManagerGetReadOnlyConfig(SINK_PARTYMODE_READONLY_CONFIG_BLK_ID, (const void **)&read_config))
    {
        ...
        else
        {
            ...
            if(BdaddrIsSame(getA2dpLinkBdAddr(a2dp_primary), sinkAvrcpGetLinkBdAddr(index)))
            {
                ...
                if(((sinkAvrcpGetPlayStatus(index) == sinkavrcp_play_status_playing) ||
                    (sinkAvrcpGetPlayStatus(index) == sinkavrcp_play_status_stopped) ||
                    (sinkAvrcpGetPlayStatus(index) == sinkavrcp_play_status_paused)) 
                        && a2dpAudioSinkMatch(a2dp_primary, sinkAudioGetRoutedAudioSink())
			&& (sinkA2dpGetStreamState(a2dp_primary) == a2dp_stream_streaming))
                {
                    ...
                    if((sinkA2dpGetStreamState(a2dp_secondary) == a2dp_stream_streaming)||(sinkA2dpGetStreamState(a2dp_secondary) == a2dp_stream_open))
                    {
                        ...
                        if((PARTYMODE_GDATA.partymode_pause.audio_source_secondary_paused)&&(sinkAvrcpGetPlayStatus((index^1)) == sinkavrcp_play_status_paused))
                        {
            ...
            {
                /* track change on primary a2dp device, check it is still streaming and audio is routed */
                if(((sinkAvrcpGetPlayStatus(index) == sinkavrcp_play_status_playing)||(sinkAvrcpGetPlayStatus(index) == sinkavrcp_play_status_stopped)||(sinkAvrcpGetPlayStatus(index) == sinkavrcp_play_status_paused))
                    && a2dpAudioSinkMatch(a2dp_secondary, sinkAudioGetRoutedAudioSink()) && sinkA2dpGetStreamState(a2dp_secondary) == a2dp_stream_streaming)
                {
                    /* track change on a2dp primary is valid, check if a2dp secondary is paused */
                    if((sinkA2dpGetStreamState(a2dp_primary) == a2dp_stream_streaming)||(sinkA2dpGetStreamState(a2dp_primary) == a2dp_stream_open))
                    {
                        /* check avrcp play status is paused */        
                        if((PARTYMODE_GDATA.partymode_pause.audio_source_primary_paused)&&(sinkAvrcpGetPlayStatus((index^1)) == sinkavrcp_play_status_paused))
                        {
...
a2dp_link_priority sinkPartyModeCheckForOtherPausedSource(a2dp_link_priority link)
{
    ...
    if(BdaddrIsSame(getA2dpLinkBdAddr(OTHER_DEVICE(link)), sinkAvrcpGetLinkBdAddr(a2dp_primary)))
    {
        ...
        if(sinkAvrcpGetPlayStatus(a2dp_primary) == sinkavrcp_play_status_paused)
    ...
    {
        /* device is connected, check its current avrcp state, if paused then return index of device */        
        if(sinkAvrcpGetPlayStatus(a2dp_secondary) == sinkavrcp_play_status_paused)
            return OTHER_DEVICE(link);
    }
