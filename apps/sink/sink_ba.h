...
/*!
    @brief isPowerOnInBroadcastMode(). This utility is used
    to determine if the power on needs to be handled in BA
...
#else
#define isPowerOnInBroadcastMode() (FALSE)
#endif

/*!
    @brief isPowerOffInBroadcastMode(). This utility is used
    to determine if the power off needs to be handled in BA
...
#else
#define isPowerOffInBroadcastMode() (FALSE)
#endif
...
/*!
    @brief sinkBaUpdatePowerOnAction(). This interface is used
    to update Power ON.action in BA mode
    @param void
...
#else
#define sinkBaUpdatePowerOnAction() ((void)(0))
#endif

/*!
    @brief sinkBroadcastAudioHandlePrompts(). This interface is used
    to play audio prompts in BA mode
    @param event the event for playing prompt
...
#else
#define sinkBroadcastAudioHandlePrompts(event) ((void)(0))
#endif

/*!
    @brief sinkBroadcatAudioAllowEvent(). This interface is used
    to determine if the event is allowed in the corresponding role
...
#else
#define sinkBroadcatAudioAllowEvent(event) (TRUE)
#endif
