...
/****************************************************************************
NAME
    MpDoesCallStateIndicateDisconnect

DESCRIPTION
    check whether the active call state is either disconnect or
    incoming, if not return false

RETURNS
    true or false depending on the the call state
*/

static bool MpDoesCallStateIndicateDisconnect(hfp_call_state call_state)
{
    return ((call_state == hfp_call_state_idle
        || call_state == hfp_call_state_incoming) ? TRUE : FALSE);
}
...
void sinkHandleCallInd(const HFP_CALL_STATE_IND_T *pInd)
{
    ...
    if(MpDoesCallStateIndicateDisconnect(CallState))
    {
