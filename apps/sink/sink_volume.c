...
{      
    ...
    if( pPlayTone && lPlayTone && sinkVolumeGetVolumeMappingforTone(pNewVolume))
...
void storeCurrentSinkVolume( void )
{
    ...
    switch( route )
    {
        ...
        case audio_route_none:
            {
                if(sinkHfpDataGetProfileStatusConnected(PROFILE_INDEX(hfp_primary_link)) && HfpLinkGetBdaddr(hfp_primary_link, &SrcAddr.taddr.addr) ) 
                {
...
{
    sink_volume_readonly_config_def_t *read_configdata = NULL;
    bool VolumeChangeCausesUnMute= FALSE;
    bool AdjustVolumeWhilstMuted= FALSE;
    /* ensure profile is connected before changing volume */
    if (priority)
    {
        if (configManagerGetReadOnlyConfig(SINK_VOLUME_READONLY_CONFIG_BLK_ID, (const void **)&read_configdata))
        {
            VolumeChangeCausesUnMute = read_configdata->VolumeChangeCausesUnMute; 
            AdjustVolumeWhilstMuted = read_configdata->AdjustVolumeWhilstMuted;
            configManagerReleaseConfig(SINK_VOLUME_READONLY_CONFIG_BLK_ID);
        }
        ...
        if ((stateManagerIsConnected() && (!sinkHfpDataGetAudioGMuted(PROFILE_INDEX(priority) )))
            || (VolumeChangeCausesUnMute)||(AdjustVolumeWhilstMuted))
        {     
...
*****************************************************************************/
static void modifyAndUpdateVolume(const volume_direction direction, const audio_output_group_t group, volume_info * const volumeToModify)
{
...
*****************************************************************************/
static bool ifA2dpAudioRoutedModifyAndUpdateVolume(const volume_direction direction, const audio_output_group_t group)
{
    if(a2dpA2dpAudioIsRouted())
    {
        ...
        {
            modifyAndUpdateVolume(direction, group, sinkA2dpGetA2dpVolumeInfoAtIndex(index));
    ...
    else if(sinkReceiverIsStreaming())
    {
        ...
        modifyAndUpdateVolume(direction, group, sinkA2dpGetA2dpVolumeInfoAtIndex(0));
        return TRUE;
    }
    return FALSE;
}
...
*****************************************************************************/
static bool ifWiredAudioRoutedModifyAndUpdateVolume(const volume_direction direction, const audio_output_group_t group)
{
    ...
    {    
        ...
        modifyAndUpdateVolume(direction, group, &analog_volume);
    ...
    if(wiredSpdifAudioIsRouted())
    {
        ...
        modifyAndUpdateVolume(direction, group, &spdif_volume);
    ...
    if(wiredI2SAudioIsRouted())
    {
        ...
        modifyAndUpdateVolume(direction, group, &i2s_volume);
...
*****************************************************************************/
static bool ifFmIsOnModifyAndUpdateVolume(const volume_direction direction, const audio_output_group_t group)
{  
    if(sinkFmIsFmRxOn())
    {
        ...
        modifyAndUpdateVolume(direction, group, &fm_volume_info);
...
*****************************************************************************/
static void modifyAndUpdateUsbVolume(const volume_direction direction, const audio_output_group_t group)
{
    ...
    if(group == audio_output_group_aux)
    {
        modifyAndUpdateVolume(direction, group, &usb_volume);
    }
    else
    {
        if(direction == increase_volume)
...
static bool ifCurrentSinkIsUsbModifyAndUpdateVolume(const volume_direction direction, const audio_output_group_t group)
{
    if(usbIfCurrentSinkIsUsb())
    {
        modifyAndUpdateUsbVolume(direction, group);
        return TRUE;
    }
    return FALSE;
}

static bool ifUsbSinkExistsModifyAndUpdateVolume(const volume_direction direction, const audio_output_group_t group)
{
...
void modifyAndUpdateRoutedAudioVolume(volume_direction direction, audio_output_group_t group)
{
    /* Try A2DP first */
    if(!sinkAccessoryVolumeRoute(direction) && ifA2dpAudioRoutedModifyAndUpdateVolume(direction, group))
        return;
    ...
    if(ifCurrentSinkIsUsbModifyAndUpdateVolume(direction, group))
        return;

    /* Try wired if has audio */
    if(ifWiredAudioRoutedModifyAndUpdateVolume(direction, group))
        return;

    /*Try FM RX*/
    if(ifFmIsOnModifyAndUpdateVolume(direction, group))
        return;

    /* Try USB */
    if(ifUsbSinkExistsModifyAndUpdateVolume(direction, group))
        return;
...
{
    VolumeSetHeadsetVolume ( ind->volume_gain , sinkTonesIsPlayLocalVolumeTone() , ind->priority ) ;
...
void sinkVolumeModifyAndUpdateRoutedAudioMainVolume(const volume_direction dir)
{
    if(increase_volume == dir)
        ...
        GSINK_VOLUME_DATA.vol_at_max = FALSE;
    modifyAndUpdateRoutedAudioVolume(dir, audio_output_group_main);
}

static void modifyAndUpdateRoutedAudioAuxVolume(const volume_direction dir)
{
    modifyAndUpdateRoutedAudioVolume(dir, audio_output_group_aux);
}

void sinkVolumeModifyAndUpdateRoutedA2DPAudioMainVolume(const volume_direction direction)
{
    ifA2dpAudioRoutedModifyAndUpdateVolume(direction, audio_output_group_main);
}
...
void sinkVolumeUpdateRoutedAudioMainAndAuxVolume(void)
{
    ...
    modifyAndUpdateRoutedAudioVolume(same_volume, audio_output_group_aux);
}
...
void sinkVolumeSetCurrentVolumeAsScaledPercentage(scaled_volume_percentage_t volume)
{
