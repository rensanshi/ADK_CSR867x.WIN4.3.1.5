...
static void handleCLMessage ( Task task, MessageId id, Message message )
{
    ...
    switch(id)
    {
        ...
        case CL_DM_LOCAL_NAME_COMPLETE:
            ...
            sinkNfcWriteChCarriersReq(BT_CAR_ID);
...
static void handleDeviceAndAudioRoutingInfoEvents(const MessageId id)
{
    ...
    MAIN_DEBUG(("Main : handleDeviceAndAudioRoutingInfoEvents() - Event %x\n",id));
    switch(id)
    {
...
static void handleUEMessage  ( Task task, MessageId id, Message message )
{
    ...
    /* If there has been an error with the configuraiton data, do not process
       any user events. */
    if (LedsInErrorMode() && (id <= EVENTS_USR_LAST_EVENT))
        return;
    ...
    switch ( id )
    {
        ...
        break;
        case (EventSysChargerStuckTimeout):
        {
            MAIN_DEBUG(("HS: Charger Stuck Timeout\n"));
            displayShowSimpleText("Charger Stuck", SINK_TEXT_TYPE_CHARGER_STATE);
      ...
      case EventSysCodecAptxConfigured:
            MAIN_DEBUG(("EventSysCodecAptxConfigured\n"));
           break;
...
static void handleHFPMessage  ( Task task, MessageId id, Message message )
{
    ...
    case HFP_VOICE_RECOGNITION_ENABLE_CFM:
        MAIN_DEBUG(("HFP_VOICE_RECOGNITION_ENABLE_CFM s[%d] w[%d]i", (((const HFP_VOICE_RECOGNITION_ENABLE_CFM_T *)message)->status ) , sinkHfpDataGetVoiceRecognitionIsActive()));
    ...
    case HFP_CALLER_ID_IND:
        {
            ...
            MAIN_DEBUG(("HFP_CALLER_ID_IND number %s", ind->caller_number));
...
static void handleAudioPluginMessage( Task task, MessageId id, Message message )
{
    ...
    switch (id)
    {
        ...
        case AUDIO_SIGNAL_DETECT_MSG:
        {
            ...
            if (stateManagerGetState() != deviceLimbo)
            {
                stream_detected = ((AUDIO_SIGNAL_DETECT_MSG_T*)message)->signal_detected;
...
/*    MAIN_DEBUG(("MSG [%x][%x][%x]\n", (int)task , (int)id , (int)&message)) ;*/
    ...
    else if (IS_VALID_NFC_CL_MSG_ID(id))
    {
