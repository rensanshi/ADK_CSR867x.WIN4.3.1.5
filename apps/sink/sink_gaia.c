...
bool gaia_get_pio_change_mask(pio_common_allbits *mask)
{
    if (mask)
    {
        *mask = GAIA_GDATA.pio_change_mask;
        return !PioCommonAllBitsIsEmpty(mask);
}
    ...
    if (GAIA_GDATA.num_mapped_pios)
    {
        ...
        while (mask && (index < GAIA_GDATA.num_mapped_pios))
        {
            if (mask & 0x1)
            {
                ...
                PioCommonAllBitsSetBit(&allbits, pio);
}
...
static uint32 gaia_translate_allbits(pio_common_allbits *allbits_mask)
{
    ...
    if (GAIA_GDATA.num_mapped_pios)
    {
        ...
        while (index-- > 0)
        {
            ...
            if (PioCommonAllBitsBitIsSet(allbits_mask, pio))
            {
                result |= 0x1;
            }
        }
...
bool gaia_get_pio_old_state(pio_common_allbits *state)
{
    if (state)
    {
        *state = GAIA_GDATA.pio_old_state;
        return !PioCommonAllBitsIsEmpty(state);
    }
...
{
    gaia_writeable_config_def_t *rw_config_data = NULL;
    bool result = FALSE;

    if (configManagerGetReadOnlyConfig(GAIA_WRITEABLE_CONFIG_BLK_ID, (const void **)&rw_config_data))
    {
       result = rw_config_data->GaiaRemainConnected;

       configManagerReleaseConfig(GAIA_WRITEABLE_CONFIG_BLK_ID);
    }
    return result;
}
...
void gaia_set_remain_connected(bool connected)
{
    ...
    if (configManagerGetWriteableConfig(GAIA_WRITEABLE_CONFIG_BLK_ID, (void **)&rw_config_data, 0))
    {
        rw_config_data->GaiaRemainConnected= connected;
        configManagerUpdateWriteableConfig(GAIA_WRITEABLE_CONFIG_BLK_ID);
    }
}
...
uint16 gaia_get_SC_PEQ_Settings_Timeout(void)
{
    ...
    /*read the read only configuration */
    if(configManagerGetReadOnlyConfig(GAIA_READ_ONLY_CONFIG_BLK_ID, (const void **)&read_config_data))
    {
        ret_val = read_config_data->StoreCurrentPEQSettingsTimeout_s;
        configManagerReleaseConfig(GAIA_READ_ONLY_CONFIG_BLK_ID);
    }
    return ret_val;
}
...
/****************************************************************************
NAME
	gaia_set_SC_PEQ_Settings_Timeout

DESCRIPTION
    Set the GAIA Store PEQ settings time out
*/
static bool gaia_set_SC_PEQ_Settings_Timeout(uint16 timeout)
{
    gaia_read_only_config_def_t *read_config_data = NULL;

    if (configManagerGetWriteableConfig(GAIA_READ_ONLY_CONFIG_BLK_ID, (void **)&read_config_data, 0))
    {
        read_config_data->StoreCurrentPEQSettingsTimeout_s = timeout;
        configManagerUpdateWriteableConfig(GAIA_READ_ONLY_CONFIG_BLK_ID);
        return TRUE;
    }
...
static uint16 get_timer_value(gaia_timer_param_t timer)
{
    ...
    case gaia_timer_language_confirm_time_s: return SinkAudioPromptsGetLanguageConfirmTime();
    ...
    case gaia_timer_store_current_peq_settings_timeout_s: return gaia_get_SC_PEQ_Settings_Timeout();
...
static bool set_timer_value(gaia_timer_param_t timer, uint16 value)
{
    ...
    case gaia_timer_language_confirm_time_s: return SinkAudioPromptsSetLanguageConfirmTime(value);
    ...
    case gaia_timer_store_current_peq_settings_timeout_s: return gaia_set_SC_PEQ_Settings_Timeout(value);
...
static bool process_user_eq_param(audio_plugin_user_eq_param_t* param, uint16 unused, uint16 gaia_cmd)
{
    if (validate_peq_payload(param, unused))
    {
        ...
        AudioSetUserEqParameter(sinkAudioGetRoutedAudioTask(), param);
                ...
                if(gaia_get_SC_PEQ_Settings_Timeout())
                {
                    MessageCancelAll(&theSink.task, EventSysGaiaEQChangesStoreTimeout);
                    MessageSendLater(&theSink.task, EventSysGaiaEQChangesStoreTimeout , 0, D_SEC(gaia_get_SC_PEQ_Settings_Timeout()));
                }
#endif
                return TRUE;
            }
...
void handleGaiaMessage(Task task, MessageId id, Message message)
{
    ...
    case  GAIA_VA_CANCEL_CFM:
        {
            GAIA_VA_CANCEL_CFM_T *cfm = (GAIA_VA_CANCEL_CFM_T *)message;
            GAIA_DEBUG(("G: GAIA_VA_CANCEL_CFM: %u\n", cfm->success));
            SinkVaHandleCancelCfm(cfm->success);
        }
        break;
...
void gaiaReportPioChange(pio_common_allbits *pio_state)
{
    ...
    if (PioCommonAllbitsXOR(&result, pio_state, &old_state))
    {
        /* Bits have changed, see if they match the change mask */
        if (PioCommonAllbitsAND(&result,&result,&change_mask))
    {
