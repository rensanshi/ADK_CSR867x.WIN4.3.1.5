...
{
#ifdef ENABLE_PEER
    attributes->master_routing_mode = peerGetTwsMasterAudioRouting();
    attributes->slave_routing_mode = peerGetTwsSlaveAudioRouting();
...
uint16 deviceManagerNumConnectedPeerDevs(void)
{
    ...
    for(priority = a2dp_primary; priority <= a2dp_secondary; priority++)
    {
        ...
        if (SinkA2dpIsInitialised() && 
            (getA2dpLinkDataDeviceId(priority) != INVALID_DEVICE_ID) && 
...
void deviceManagerExtractDeviceLinkKey(const CL_SM_GET_AUTH_DEVICE_CFM_T *cfm)
{
    ...
    {
        sprintf(num_string, "%04x%04x%04x%04x",
                cfm->link_key[7],
                cfm->link_key[6],
                cfm->link_key[5],
                cfm->link_key[4]
                );
        displayShowText(num_string, 16, FALSE, 0, 0, 0, 0, SINK_TEXT_TYPE_CALLER_INFO);
        
        DEV_DEBUG(("DEV: LINK KEY - size[0x%x] addr[%x:%x:%lx] Key[%s", cfm->size_link_key, cfm->bd_addr.uap, cfm->bd_addr.nap, cfm->bd_addr.lap, num_string));
        ...
        displayShowText(num_string, 16, TRUE, 1000, 2000, FALSE, 0, SINK_TEXT_TYPE_DEVICE_STATE);
        
        DEV_DEBUG(("%s]\n", num_string));
    }
    
    /* Disable showing link keys again until another event is triggered */
