...
DESCRIPTION
    Initialise the A2DP TTP Latency parameters

RETURNS
    void
**************************************************************************/
void sinkA2dpTtpLatencyInit(void)
{
    a2dp_config_def_t *a2dp_config_data = NULL;

    if(configManagerGetReadOnlyConfig(A2DP_CONFIG_BLK_ID, (const void **)&a2dp_config_data))
    {
        AudioConfigSetTtpLatency(a2dp_config_data->minimum_ttp_latency_ms,
                                 a2dp_config_data->target_ttp_latency_ms,
                                 a2dp_config_data->maximum_ttp_latency_ms);
        configManagerReleaseConfig(A2DP_CONFIG_BLK_ID);
    }
}
...
static uint16 a2dpMapMusicModeEnhancementsToA2dpData(a2dp_session_config_def_t *a2dp_config_data)
{
    ...
    if (a2dp_config_data->audio_enhancement_enable) music_mode_enhancements |= MUSIC_CONFIG_DATA_VALID;
...
static void a2dpMapMusicModeEnhancementsToSessionData(uint16 music_mode_enhancements, a2dp_session_config_def_t *a2dp_config_data)
{
    ...
    a2dp_config_data->audio_enhancement_enable = MusicEnhancementBypassed(music_mode_enhancements, MUSIC_CONFIG_DATA_VALID);
}
...
bool sinkA2dpRenegotiateCodecsIfNotSupported(a2dp_link_priority a2dp_source_index)
{
...
bool sinkA2dpSourceHasUnsupportedCodec(a2dp_link_priority a2dp_source_index)
{
...
/*************************************************************************
DESCRIPTION
    If the SEID negotiated with the currently connected AV source
    is disabled by the sink device, then attempt to reconnect to the AV source.
...
static void reconnectAvSource(void)
{
    ...
    {
        if(PeerReconnectAgOnPeerConnectionEnabled() || sinkA2dpSourceHasUnsupportedCodec(av_id))
        {
...
static void handleA2dpCodecConfigureIndFromPeer (A2DP_CODEC_CONFIGURE_IND_T* ind)
{
    ...
    switch (current_source)
    {
        ...
        case RELAY_SOURCE_A2DP:
            ...
            {
                uint8 device_id = getA2dpLinkDataDeviceId(priority);
                uint8 stream_id = getA2dpLinkDataStreamId(priority);
...
void InitA2dp(void)
{
    ...
    setTWSRoutingMode(tws_master, peerGetTwsMasterAudioRouting());
    setTWSRoutingMode(tws_slave, peerGetTwsSlaveAudioRouting());
#endif
...
bool getA2dpIndexFromPlugin(Task audio_plugin, uint16 *Index)
{
    ...
    if (!audio_plugin || !SinkA2dpIsInitialised())
    {
        return FALSE;
    }
    ...
    for_all_a2dp(i)
    {
        ...
        if(getA2dpStatusFlag(CONNECTED, i))
        {
            if (getA2dpLinkDataSeId(i) && (getA2dpPlugin(getA2dpLinkDataSeId(i)) == audio_plugin))
            {
                *Index = i;
                return TRUE;
            }
...
/*        A2DP_DEBUG(("A2dp: getA2dpStreamData - peer=%u connected=%u\n",A2DP_DATA.peer_device[priority], getA2dpStatusFlag(CONNECTED, priority))); */
        if(getA2dpStatusFlag(CONNECTED, priority))
        {
            *state = A2dpMediaGetState(getA2dpLinkDataDeviceId(priority), getA2dpLinkDataStreamId(priority)) ;
            *sink  = A2dpMediaGetSink(getA2dpLinkDataDeviceId(priority), getA2dpLinkDataStreamId(priority)) ;
...
/*        A2DP_DEBUG(("A2dp: getA2dpStreamRole - peer=%u connected=%u\n",A2DP_DATA.peer_device[priority],getA2dpStatusFlag(CONNECTED, priority))); */
        if(getA2dpStatusFlag(CONNECTED, priority))
        {
            *role = A2dpMediaGetRole(getA2dpLinkDataDeviceId(priority), getA2dpLinkDataStreamId(priority)) ;
        ...
        else
        {
            ...
            if(getA2dpIndex(DeviceId, (uint16*)&priority))
            {
                ...
                if(sinkPartymodeIsEnabled() && sinkPartymodeGetOperatingMode())
                {
                    ...
                    MessageSendLater(&theSink.task,(EventSysPartyModeTimeoutDevice1 + priority),0,D_SEC(sinkPartymodeGetMusicTimeOut()));
                    /* set paused flag */
                ...
                sinkA2dpSetA2dpVolumeAtIndex(priority, attributes.a2dp.volume);
                setA2dpLinkDataClockMismatchRate(priority, attributes.a2dp.clock_mismatch);
                ...
                if ((getA2dpPeerRemoteDevice(priority) == remote_device_peer ) ||
                    (getA2dpStatusFlag(CONNECTED, a2dp_primary) && getA2dpStatusFlag(CONNECTED, a2dp_secondary)))
                {
                    setA2dpPeerLinkLossReconnect(FALSE);
                }
                A2DP_DEBUG(("Remote device type = %u\n",getA2dpPeerRemoteDevice(priority)));
                A2DP_DEBUG(("Remote device features = 0x%x\n",getA2dpPeerRemoteFeatures(priority)));
                A2DP_DEBUG(("Remote device version = 0x%X\n",getA2dpLinkDataPeerVersion(priority)));
#endif
               /* if connected as a result of an inquiry connect, stop further attempts */
                ...
                linklossUpdateManagement(getA2dpLinkBdAddr(priority));
...
#endif
                    if ((getA2dpSuspendState(priority) == a2dp_local_suspended) ||
                        (sinkA2dpOpenMediaOnConnection()))
                    {
            ...
            if(avrcpAvrcpIsEnabled())
            {
                ...
                if(getA2dpIndex(DeviceId, (uint16*)&priority))
                {
                    ...
                    if ((getA2dpPeerRemoteDevice(priority) == remote_device_peer) && (getA2dpPeerRemoteFeatures(priority) & remote_features_peer_avrcp) && locally_initiated)
                    {
                        sinkAvrcpConnect(getA2dpLinkBdAddr(priority), DEFAULT_AVRCP_NO_CONNECTION_DELAY);
                    }
                    else
#endif
                    {
                        sinkAvrcpConnect(getA2dpLinkBdAddr(priority), DEFAULT_AVRCP_1ST_CONNECTION_DELAY);
                    }
                }
            }
#endif
...
void connectA2dpStream (a2dp_link_priority priority, uint16 delay)
{
    ...
    if (!delay)
    {
        if (SinkA2dpIsInitialised() && getA2dpStatusFlag(CONNECTED, priority))
        {
#ifdef ENABLE_PEER
            if (getA2dpPeerRemoteDevice(priority) == remote_device_unknown)
            ...
            else if (getA2dpPeerRemoteDevice(priority) == remote_device_nonpeer)
#endif
            {   /* Open media channel to AV Source */
                A2DP_DEBUG(("local device is non-peer (AV Source)\n"));
                if (A2dpMediaGetState(getA2dpLinkDataDeviceId(priority), 0) == a2dp_stream_idle)
                {
...
{
    if (SinkA2dpIsInitialised() && (getA2dpPeerRemoteDevice(priority) != remote_device_peer))
    {
        bool status = FALSE;
        a2dp_codec_settings * codec_settings = NULL;
        ...
        codec_settings = A2dpCodecGetSettings(getA2dpLinkDataDeviceId(priority), getA2dpLinkDataStreamId(priority));
        if(codec_settings != NULL)
        {
        ...
        {
            /* ensure media is streaming and the avrcp channel is that requested to be paused */
            if ((sinkAvrcpIsConnected(i))&&(getA2dpStatusFlag(CONNECTED, priority))&&
                (BdaddrIsSame(getA2dpLinkBdAddr(priority), sinkAvrcpGetLinkBdAddr(i))))
            {
        ...
        if(!status)
        {
            if(!A2dpMediaSuspendRequest(getA2dpLinkDataDeviceId(priority), getA2dpLinkDataStreamId(priority)))
            {
                /* suspend failed so close media streaming */
                A2dpMediaCloseRequest(getA2dpLinkDataDeviceId(priority), getA2dpLinkDataStreamId(priority));
            }
        }
...
void  a2dpSuspendNonRoutedStream(a2dp_link_priority priority)
{
    ...
    if (a2dpGetPeerIndex(&index)
        ...
        && !getA2dpStatusFlag(PLAYING, priority))
    {
...
a2dp_suspend_state sinkA2dpGetA2dpLinkSuspendState(a2dp_link_priority priority)
{
    return (SinkA2dpIsInitialised() ? getA2dpSuspendState(priority) : a2dp_not_suspended);
}


bool sinkA2dpIsA2dpLinkSuspended(a2dp_link_priority priority)
{
    return (sinkA2dpGetA2dpLinkSuspendState(priority) != a2dp_not_suspended);
}
...
**************************************************************************/
static bool resumeUsingAvrcpPlay(a2dp_link_priority priority)
{
    bool status_avrcp = FALSE;
    uint16 index;
    ...
    {
        /* ensure media is streaming and the avrcp channel is that requested to be paused */
        if ((sinkAvrcpIsConnected(index))&&(getA2dpStatusFlag(CONNECTED, priority))&&
            (BdaddrIsSame(getA2dpLinkBdAddr(priority), sinkAvrcpGetLinkBdAddr(index))))
        {
            /* attempt to resume playing the a2dp stream */
            status_avrcp = sinkAvrcpPlayPauseRequest(index,AVRCP_PLAY);
            break;
        }
    }
    return status_avrcp;
...
bool ResumeA2dpStream(a2dp_link_priority priority)
{
    ...
    if (SinkA2dpIsInitialised() && (getA2dpPeerRemoteDevice(priority) != remote_device_peer))
    {
        ...
        if(A2dpSignallingGetState(getA2dpLinkDataDeviceId(priority)) == a2dp_signalling_connected)
        {
            ...
            {
#ifdef ENABLE_AVRCP
                if(resumeUsingAvrcpPlay(priority))
                {
                    /* update state */
                ...
                /* if not successful in resuming play via avrcp try a media start instead */
                {
                    ...
                    A2dpMediaStartRequest(getA2dpLinkDataDeviceId(priority), getA2dpLinkDataStreamId(priority));
        ...
        else
        {
            ...
            sinkA2dpSignallingConnectRequest(getA2dpLinkBdAddr(priority));
        }
    }
...
**************************************************************************/
void handleA2DPStoreCurrentEqBank (uint16 eq_bank)
{
    if(sinkMusicProcessingUpdateEqBank(eq_bank))
    {
        A2DP_DEBUG(("A2dp: Update eq bank to %u\n", eq_bank));
#ifdef ENABLE_PEER
...
**************************************************************************/
void handleA2DPStoreEnhancements(uint16 new_enhancements_bypassed)
{
            ...
            for (param_type = 0; param_type < eq_param_max_param_types; param_type++)
            {
                ...
                switch (param_type)
                {
                    case eq_param_type_filter:
                        param.value = PEQ->bands[band].filter;
                        break;

                    case eq_param_type_cutoff_freq:
                        param.value = PEQ->bands[band].freq;
                        break;

                    case eq_param_type_gain:
                        param.value = PEQ->bands[band].gain;
                        break;

                    case eq_param_type_q:
                        param.value = PEQ->bands[band].Q;
                        break;
...
bool a2dpGetNextAvBdAddress(const bdaddr *bd_addr , bdaddr *next_bdaddr )
{
    ...
    if(getA2dpIndexFromBdaddr (bd_addr, &index))
    {
        ...
        if(next_bdaddr && getA2dpStatusFlag(CONNECTED, other_device_link_priority) &&
           (getA2dpPeerRemoteDevice(other_device_link_priority) != remote_device_peer))
        {
             a2dpBdAddr = getA2dpLinkBdAddr(other_device_link_priority);
...
void handleA2DPLatencyReport (Task audio_plugin, bool estimated, uint16 latency)
{
    ...
    if (getA2dpIndexFromPlugin(audio_plugin, &index))
    {
        ...
        if (estimated)
        {
            A2dpMediaAvSyncDelayResponse(getA2dpLinkDataDeviceId(index), getA2dpLinkDataStreamId(index), latency);
        }
        else
        {
...
static void sinkUpdateCodecConfiguredToApp( uint8 deviceId,uint8 streamId)
{
    ...
    switch(codec_settings->seid)
    {
        ...
        case APTXHD_SEID:
            MessageSend ( sinkGetMainTask(), EventSysCodecAptxConfigured , 0 ) ;
        break;
...
a2dp_stream_state sinkA2dpGetStreamState(a2dp_link_priority priority)
{
    if (SinkA2dpIsInitialised())
    {
        if(getA2dpStatusFlag(CONNECTED, priority))
        {
            return A2dpMediaGetState(getA2dpLinkDataDeviceId(priority), getA2dpLinkDataStreamId(priority));
        }
    }
...
a2dp_role_type sinkA2dpGetRoleType(a2dp_link_priority priority)
{
    if (SinkA2dpIsInitialised())
    {
        if(getA2dpStatusFlag(CONNECTED, priority))
        {
            return A2dpMediaGetRole(getA2dpLinkDataDeviceId(priority), getA2dpLinkDataStreamId(priority)) ;
        }
    }
...
static bool isA2dpLinkPlaying(a2dp_link_priority a2dp_index)
{
...
static bool isA2dpLinkResumable(a2dp_link_priority a2dp_index)
{
...
RETURNS
    void
*/
void sinkA2dpIndicateCodec(uint8 seid)
{
    A2DP_DEBUG(("sinkA2dpIndicateCodec seid %d prev seid\n", seid));
    ...
    switch(seid)
    {
        ...
        case APTX_SEID:
            ...
            break;
        case APTXHD_SEID:
            MessageSend ( &theSink.task , EventSysCodecAptxHDInUse , 0 ) ;
            break;
#ifdef INCLUDE_FASTSTREAM
        case FASTSTREAM_SEID:
...
bool sinkA2dpIsA2dpSinkPlaying(a2dp_link_priority a2dp_index)
{
...
bool sinkA2dpIsA2dpSinkRoutable(a2dp_link_priority a2dp_index)
{
