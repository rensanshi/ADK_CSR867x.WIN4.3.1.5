...
RETURNS
    void
**************************************************************************/
void usbUpdateDataLines(usb_attached_status attached_status, uint16 dp, uint16 dm)
{
...
RETURNS
    void
*/
power_boost_enable sinkUsbGetBoost(sink_charge_current_t *usb_limits)
{
#ifdef ENABLE_CHARGER_V2
    UNUSED(usb_limits);
    return power_boost_disabled;
#else
...
RETURNS
    TRUE if limits were obtained, FALSE if limits not available
*/
bool usbGetChargeCurrent(sink_charge_current_t* usb_limit)
{
    ...
    switch(UsbAttachedStatus())
    {
        case HOST_OR_HUB:
            ...
            if(sinkUsbIsSuspended())
            {
                ...
                {
                    *usb_limit = usb_data->charge[usb_i_susp_db];
                }
                else
                {
                    *usb_limit = usb_data->charge[usb_i_susp];
                }
            }
            else if(powerManagerIsChargerFullCurrent())
            {
                ...
                if(!sinkUsbIsEnumerated())
                {
                    *usb_limit = usb_data->charge[usb_i_att];
                }
                else
                {
                    *usb_limit = usb_data->charge[usb_i_conn];
                }
            }
            else
            {
                ...
                if(!sinkUsbIsEnumerated())
                {
                    *usb_limit = usb_data->charge[usb_i_att_trickle];
                }
                else
                {
                    *usb_limit = usb_data->charge[usb_i_conn_trickle];
                }
            }
            ret_val = TRUE;
            break;
        ...
        case DEDICATED_CHARGER:
            USB_DEBUG(("Dedicated Charger Port%s\n", sinkUsbIsVbusOkay() ? "" : " Limited"));
            if(sinkUsbIsVbusOkay())
            {
                *usb_limit = usb_data->charge[usb_i_dchg];
            }
            else
            {
                *usb_limit = usb_data->charge[usb_i_lim];
            }
            ret_val = TRUE;
            break;
        ...
        case CHARGING_PORT:
            ...
            if(sinkUsbIsVbusOkay())
            {
                *usb_limit = usb_data->charge[usb_i_chg];
            }
            else
            {
                *usb_limit = usb_data->charge[usb_i_lim];
        ...
        case NON_COMPLIANT_CHARGER:
            USB_DEBUG(("Non-compliant charger\n"));
            if(sinkUsbIsVbusOkay())
            {
                if (!usbGetNonCompliantChargeCurrent(usb_limit))
                {
                    USB_DEBUG((" - unrecognised\n"));
                    *usb_limit = usb_data->charge[usb_i_pchg];
                }
            }
            else
            {
        ...
        default:
            ...
            {
                *usb_limit = usb_data->charge[usb_i_disc];
                 ret_val = TRUE;
            }
            break;
    }
    ...
    if (ret_val)
    {
    USB_DEBUG(("USB limits: current %u, config %u, boost %u, charge %u, vsel %s, power_off %u, disable_leds %u\n",
                   usb_limit->current, usb_limit->charger_config, sinkUsbGetBoost(usb_limit), usb_limit->charge, usb_limit->vsel == 1 ? "VBAT" : "VBYP",
    ...
    if(usbMicOrSpeakerEnabled() && sink)
    {
        ...
        if(usbAudioIsAttached())
        {
            ...
            if(sinkAudioGetRoutedAudioSink() != sink)
            {
                ...
                uint32 sampleFreq;
                AUDIO_MODE_T mode;
                const usb_plugin_info* plugin_info = usbAudioGetPluginInfo(&plugin, sinkUsbconfigGetPluginType(), sinkUsbconfigGetPluginIndex());
                ...
                else
                {
                    ...
                    USB_DEBUG(("USB: Connect voice %p %p\n",
                           (void *) sink, 
...
Sink usbGetAudioSink(void)
{
    ...
    if((sinkUsbconfigIsAudioAlwaysOn()) || ((usbAudioIsAttached()) && (sinkUsbIsSpeakerActive())))
    {
        ...
        USB_DEBUG(("USB: usbGetAudioSink sink %p, mic enabled = %x, spk enabled = %x\n", (void*)sink ,  USB_CLASS_ENABLED(USB_DEVICE_CLASS_TYPE_AUDIO_MICROPHONE), USB_CLASS_ENABLED(USB_DEVICE_CLASS_TYPE_AUDIO_SPEAKER) ));
    }
    /* USB not attached */
    else
        USB_DEBUG(("USB: usbGetAudioSink sink %p, mic enabled = %x, spk enabled = %x, speaker active = %x\n", (void*)sink ,  USB_CLASS_ENABLED(USB_DEVICE_CLASS_TYPE_AUDIO_MICROPHONE), USB_CLASS_ENABLED(USB_DEVICE_CLASS_TYPE_AUDIO_SPEAKER), USB_RUNDATA.spkr_active ));
