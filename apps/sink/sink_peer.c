...
**************************************************************************/
static void peerChangeAudioRouting(void)
{
    if(peerGetPermittedRoutingModes() != 0)
    {
...
void peerHandleStatusChangeCmd (PeerStatusChange peer_status_change)
{
    ...
    if (a2dpGetPeerIndex(&peer_id))
    {
        ...
        if(!peerIsSingleDeviceOperationEnabled() && findCurrentA2dpSource(&priority)
...
void peerRequestUserEqSetings(void)
{
    ...
    {    
        ...
        if(findCurrentA2dpSource(&priority) && (getA2dpPeerRemoteDevice(priority) == remote_device_peer))
        {
            uint16 peerAvrcpIndex;
            for_all_avrcp(peerAvrcpIndex)
            {
...
void peerUpdateLocalStatusOnCallEnd(void)
{
    ...
    if( !peerIsSingleDeviceOperationEnabled() && findCurrentStreamingA2dpSource(&priority) && !a2dpIsIndexPeer(priority) )
    {
...
static MessageId processReceivedSynchronisedPeerVolumeEvent(uint8 * payload)
{
    ...
    sinkVolumeSetCurrentVolumeAsScaledPercentage(volume);
...
static PeerAncEventType getAncEventPayload(MessageId anc_event)
{
    ...
    switch(anc_event)
    {
        ...
        case EventUsrAncLeakthroughMode:
        	peerAncEvent = ANC_SET_LEAKTHROUGH;
        	break;

        case EventUsrAncActiveMode:
        	peerAncEvent = ANC_SET_ACTIVE;
        	break;
...
static MessageId getPeerGeneratedMessageIdFromAncEvent(MessageId anc_event)
{
    ...
    switch(anc_event)
    {
        ...
        case EventUsrAncLeakthroughMode:
        	PeerGeneratedEvent = EventSysPeerGeneratedAncLeakthroughMode;
        	break;

        case EventUsrAncActiveMode:
        	PeerGeneratedEvent = EventSysPeerGeneratedAncActiveMode;
        	break;
...
void peerSendAncMode(void)
{
	...
	ancEvent = ((sinkAncIsModeActive()) ? EventUsrAncActiveMode : EventUsrAncLeakthroughMode);
...
bool peerDisconnectLowestPriorityAG(void)
{
    ...
    if((deviceManagerNumConnectedPeerDevs() == 0)&&(deviceManagerNumConnectedDevs() == MAX_A2DP_CONNECTIONS))
    {
                ...
                if(getA2dpStatusFlag(CONNECTED, a2dp_index))
                {
                    ...
                    if(sinkAudioGetRoutedAudioSink() == A2dpMediaGetSink(getA2dpLinkDataDeviceId(a2dp_index), getA2dpLinkDataStreamId(a2dp_index)))
                    {
                        ...
                        if(a2dp_index == a2dp_secondary)
                        {
                            ...
                            deviceManagerDisconnectAG(getA2dpLinkBdAddr(a2dp_primary));
        ...
        deviceManagerDisconnectAG(getA2dpLinkBdAddr(a2dp_secondary));
        /* disconnect initiated */
        return TRUE;
    }
...
*/
PeerTwsAudioRouting peerGetTwsMasterAudioRouting(void)
{
    sink_peer_readonly_config_def_t *read_data = NULL;
    PeerTwsAudioRouting tws_master_audio_routing = PEER_TWS_ROUTING_STEREO;
...
*/
PeerTwsAudioRouting peerGetTwsSlaveAudioRouting(void)
{
    sink_peer_readonly_config_def_t *read_data = NULL;
    PeerTwsAudioRouting tws_slave_audio_routing = PEER_TWS_ROUTING_STEREO;
