...
/****************************************************************************
NAME    
    SinkAudioPromptsGetLanguageConfirmTime
    
DESCRIPTION
    Get the audio prompts language confirm time
*/
uint16 SinkAudioPromptsGetLanguageConfirmTime(void)
{
    sink_audioprompts_readonly_config_def_t *ro_data = NULL;
    uint16 time = 0;
    ...
    if (configManagerGetReadOnlyConfig(SINK_AUDIOPROMPTS_READONLY_CONFIG_BLK_ID, (const void **)&ro_data))
    {
        time = ro_data->LanguageConfirmTime_s;
        configManagerReleaseConfig(SINK_AUDIOPROMPTS_READONLY_CONFIG_BLK_ID);
    }
    return time;
}

bool SinkAudioPromptsSetLanguageConfirmTime(uint16 time)
{
    sink_audioprompts_readonly_config_def_t *ro_data = NULL;

    if (configManagerGetWriteableConfig(SINK_AUDIOPROMPTS_READONLY_CONFIG_BLK_ID, (void **)&ro_data, 0))
    {
        ro_data->LanguageConfirmTime_s = time;
        configManagerUpdateWriteableConfig(SINK_AUDIOPROMPTS_READONLY_CONFIG_BLK_ID);
        return TRUE;
    }
...
void AudioPromptSelectLanguage( void )
{
    ...
    uint16 delay = SinkAudioPromptsGetLanguageConfirmTime();
