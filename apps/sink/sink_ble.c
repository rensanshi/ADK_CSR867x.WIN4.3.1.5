...
void sinkBleSimplePairingCompleteInd(const CL_SM_BLE_SIMPLE_PAIRING_COMPLETE_IND_T *ind)
{
    ...
    if (cid && (ind->status == success))
    {
        ...
        {
            /* Remove the device information, else with many receivers we can soon hit the boundary of persistence store */
            ConnectionSmDeleteAuthDeviceReq(TYPED_BDADDR_PUBLIC, &ind->tpaddr.taddr.addr);
        }
        else
        {
...
bool SinkBleConnectionParameterIsOutOfRange(const CL_DM_BLE_ACCEPT_CONNECTION_PAR_UPDATE_IND_T *ind)
{
    ...
    connection = gattClientFindByCid(cid);
	/* If there is no connection, just ignore the update */
	if(!connection)
	    return TRUE;
