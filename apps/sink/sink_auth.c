...
{	
    ...
    if ((remote_bdaddr->transport == TRANSPORT_BLE_ACL) && 
        ((sinkBleGapIsBondable() || ind->sm_over_bredr)))
    {
        can_pair = TRUE;
        /* We require KEY distribution either:
             1. For non-broadcast audio mode
             2. Or in broadcast audio mode, but for peripheral link
             3. Or for broadcaster having random resolvable address */
        if(sinkBroadcastAudioIsActive() && gapBaGetAssociationInProgress())
        {
             key_dist = (remote_bdaddr->taddr.type == TYPED_BDADDR_RANDOM) ? 
                            KEY_DISTRIBUTION_RANDOM : KEY_DIST_NONE;
        }
        else
        {
...
bool AuthCanSinkPair ( void )
{
    ...
    if (sinkDataIsSecureParingEnabled())
    {
        ...
        if ((stateManagerGetState() == deviceConnDiscoverable)||(sinkInquiryCheckInqActionSub()))
        {
            lCanPair = TRUE ;
            AUTH_DEBUG(("auth: is ConnDisco\n")) ;
        }
#ifdef ENABLE_PARTYMODE
        else if(sinkPartymodeIsEnabled())
        {
            lCanPair = TRUE ;
            AUTH_DEBUG(("auth: allow PartyMode pairing\n")) ;
        }
#endif   
        else if (sinkNfcIsSecondAGPairingAllowed())
        {
            lCanPair = TRUE ;
            AUTH_DEBUG(("auth: NFC Tag Read  Second AG allowed to pair\n")) ;
        }
    }
    else
    {
...
{		
    if(AuthCanSinkPair() && sinkDataGetAuthConfirmationFlag())
	{
		AUTH_DEBUG(("auth: Accepted Confirmation Req\n")) ;
		ConnectionSmUserConfirmationResponse(sinkDataGetSMConfirmationAddress(), TRUE);
     }
	else
     {
...
void sinkPairingRejectRes( void )
{
	...
	{	
		AUTH_DEBUG(("auth: Rejected Confirmation Req\n")) ;
		ConnectionSmUserConfirmationResponse(sinkDataGetSMConfirmationAddress(), FALSE);
	}
	else
	{
		AUTH_DEBUG(("auth: Invalid state for confirmation\n"));
