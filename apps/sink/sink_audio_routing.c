...
static bool audioRouteAudioFromTWSPeer(void)
{
    ...
    if(peerIsDeviceSlaveAndStreaming() && (peerIsRemotePeerInCall() == FALSE))
    {
        ...
        {
            return audioRouteA2dpStreamIfAvailable((a2dp_link_priority)peer_index);
        }
    }
    return FALSE;
}
...
void temporaryEnableAudioAmp(void)
{
...
static void audioPostRoutingAudioConfiguration(void)
{
    ...
    /* If A2DP1 or A2DP2, deteremine if AVRCP is available for that source and
        update the currently active AVRCP connection appropriately */
    avrcpSetInstanceBasedOnSourceRequested(sinkAudioGetRequestedAudioSource());
...
{
    sinkCancelAndSendLater(EventSysCheckAudioRouting, SINK_AUDIO_ROUTING_DELAY_MS);
}
...
void audioRoutingProcessUpdateMsg(void)
{
    ...
    AUD_DEBUG(("AUD: audioUpdateAudioRouting %p", (void *) sinkAudioGetRoutedAudioSink()));
    AUD_DEBUG((", allocations remaining %u\n", VmGetAvailableAllocations()));
...
void audioUpdateAudioRoutingAfterDisconnect(void)
{
    ...
    delay = sinkAudioSourceDisconnectDelay();
    if(delay > 0)
    {
        sinkCancelAndSendLater(EventSysCheckAudioRouting, (uint32)delay);
    }
    else
    {
        audioUpdateAudioRouting();
...
static audio_sources audioGetHighestPriorityAudioSourceAvailable(void)
{
    audio_sources source = sinkAudioGetAudioSourcePriority1();

    while(FALSE == audioIsAudioSourceRoutable(source))
    {
        source = getNextSourceInSequence(source);

        if(sinkAudioGetAudioSourcePriority1() == source)
        {
...
{
    return (audioIsCallWithoutSco() && !audioRouteIsMixingOfVoiceAndAudioEnabled());
}
    ...
    if(isAudioRoutingPermitted(source))
    {
        switch (source)
        {
            ...
            case audio_source_a2dp_1:
                routing_success = audioRouteA2dpStreamIfAvailable(getA2dpPriority());
                AUD_DEBUG(("AUD: As primary A2DP source, attempt to route A2DP link %d\n", getA2dpPriority()));
                break;

            case audio_source_a2dp_2:
                routing_success = audioRouteA2dpStreamIfAvailable(invertA2dpPriority(getA2dpPriority()));
                AUD_DEBUG(("AUD: As secondary A2DP source, attempt to route A2DP link %d\n", invertA2dpPriority(getA2dpPriority())));
                break;
...
static bool audioRouteA2dpStreamIfAvailable(a2dp_link_priority priority)
{
    ...
    if(sinkA2dpIsA2dpSinkRoutable(priority) && (!isAudioGated(audio_gate_a2dp)))
    {
        if(sinkA2dpIsA2dpLinkSuspended(priority) == FALSE || ResumeA2dpStream(priority))
        {
            if(sinkBroadcastIsReadyForRouting() && sinkBroadcastIsA2dpCodecSupported(priority))
            {
                a2dp_routed = TRUE;
                if(!a2dpAudioSinkMatch(priority, sinkAudioGetRoutedAudioSink()))
                {
                /* Inform VA to Abort session as A2DP audio need to be routed .*/
                   SinkVaAbortOnA2dpStream();
                   a2dp_routed = routeAudioSource(getA2dpSourceFromPriority(priority));
                }
            }
        }
    }
...
{
    if(!sinkA2dpIsA2dpLinkSuspended(priority))
    {
        if((!isActiveCallOnTheSameDevice(priority)) || (!audioRouteIsAutoSuspendOnCallEnabled()))
        {
...
static void audioDisconnectInstance(audio_instance_t instance_to_disconnect)
{
    ...
    {
        AUD_DEBUG(("AUD: disconnect task %p\n", (void *)instance_to_disconnect)) ;
...
audio_route_available sinkAudioRouteAvailable(void)
{
    ...
    if(sinkAudioIsAudioRouted())
    {
        ...
        else if( usbAudioSinkMatch(routed_audio))
        {
...
static bool a2dpMediaStartPriority(a2dp_link_priority priority)
{
    if(sinkA2dpGetStreamState(priority) == a2dp_stream_open
                    && sinkA2dpIsA2dpLinkSuspended(priority) == FALSE
                    && sinkA2dpGetRoleType(priority) == a2dp_sink)
    {
        if(A2dpSignallingGetState(getA2dpLinkDataDeviceId(priority)) == a2dp_signalling_connected)
        {
            A2dpMediaStartRequest(getA2dpLinkDataDeviceId(priority), getA2dpLinkDataStreamId(a2dp_primary));
        }
        return TRUE;
    }
    return FALSE;
...
{
    if(!a2dpMediaStartPriority(a2dp_primary))
    {
        a2dpMediaStartPriority(a2dp_secondary);
    }
}
...
static audio_sources getNextSourceInSequence(audio_sources source)
{
    ...
    if (configManagerGetReadOnlyConfig(SINK_AUDIO_READONLY_CONFIG_BLK_ID, (const void **)&read_data))
    {
        if((read_data->seqSourcePriority1 == source) &&
                (read_data->seqSourcePriority2))
        {
            next_source = (audio_sources)read_data->seqSourcePriority2;
        }
        else if((read_data->seqSourcePriority2 == source) &&
                (read_data->seqSourcePriority3))
        {
            next_source = (audio_sources)read_data->seqSourcePriority3;
        }
        else if((read_data->seqSourcePriority3 == source) &&
                (read_data->seqSourcePriority4))
        {
            next_source = (audio_sources)read_data->seqSourcePriority4;
        }
        else if((read_data->seqSourcePriority4 == source) &&
                (read_data->seqSourcePriority5))
        {
            next_source = (audio_sources)read_data->seqSourcePriority5;
        }
        else if((read_data->seqSourcePriority5 == source) &&
                (read_data->seqSourcePriority6))
        {
            next_source = (audio_sources)read_data->seqSourcePriority6;
        }
        else if((read_data->seqSourcePriority6 == source) &&
                (read_data->seqSourcePriority7))
        {
            next_source = (audio_sources)read_data->seqSourcePriority7;
        }
        else if(read_data->seqSourcePriority1)
        {
            next_source = (audio_sources)read_data->seqSourcePriority1;
        }
...
static bool audioIsAudioSourceRoutable(audio_sources source)
{
    ...
    switch(source)
    {
        ...
        case audio_source_USB:
            ...
            break;
        case audio_source_a2dp_1:
            is_routable = audioIsA2dpAudioRoutable(getA2dpPriority());
            AUD_DEBUG(("AUD: Is A2DP link %d routable?\n", getA2dpPriority()));
            break;
        case audio_source_a2dp_2:
            is_routable = audioIsA2dpAudioRoutable(invertA2dpPriority(getA2dpPriority()));
            AUD_DEBUG(("AUD: Is A2DP link %d routable?\n", invertA2dpPriority(getA2dpPriority())));
            break;
        case audio_source_ba:
...
static uint16 getAvrcpLinkIndexOfA2dpLink(a2dp_link_priority a2dp_link_index)
{
...
static bool isA2dpLinkHighestPriorityBasedOnAvrcpState(a2dp_link_priority pre_override_index)
{
    a2dp_link_priority post_override_index = pre_override_index;
...
INPUTS
    priority  a2dp_link_priority type.
...
static bool audioIsA2dpAudioRoutable(a2dp_link_priority priority)
{
    ...
    if(sinkA2dpIsA2dpSinkRoutable(priority))
    {
        is_routable = TRUE;

        if(routedA2dpShouldConsiderAvrcpState())
        {
            is_routable = isA2dpLinkHighestPriorityBasedOnAvrcpState(priority);
        }
    }

    AUD_DEBUG(("audioIsA2dpAudioRoutable(0x%X) %X\n", priority, is_routable));
    return is_routable;
}
...
static audio_sources getNextRoutableSource(audio_sources current_source)
{
    ...
    {
        current_source = sinkAudioGetAudioSourcePriority1();
    }
...
static a2dp_link_priority getA2dpPriorityIndexFromAudioSource(audio_sources a2dp_source)
{
    if(audio_source_a2dp_1 == a2dp_source)
    {
        return a2dp_primary;
    }
    else
    {
        return a2dp_secondary;
    }
}
...
static void setA2dpOverridePriority(audio_sources a2dp_source)
{
    AUDIO_GDATA.a2dp_priority_override = getA2dpPriorityIndexFromAudioSource(a2dp_source);
}
...
static a2dp_link_priority getA2dpPriority(void)
{
    ...
    {
        return getA2dpPriorityOverride();
    }
}

static a2dp_link_priority getA2dpPriorityOverride(void)
{
    return AUDIO_GDATA.a2dp_priority_override;
}
...
static void sinkAudioConfigurePcmInputs(sink_audio_readonly_config_def_t *config)
{
    ...
    if ((config->seqSourcePriority1 == audio_source_I2S) ||
        (config->seqSourcePriority2 == audio_source_I2S) ||
        (config->seqSourcePriority3 == audio_source_I2S) ||
        (config->seqSourcePriority4 == audio_source_I2S) ||
        (config->seqSourcePriority5 == audio_source_I2S) ||
        (config->seqSourcePriority6 == audio_source_I2S) ||
        (config->seqSourcePriority7 == audio_source_I2S))
    {
        if (config->UseAltPcm)
        {
            AudioConfigSetPcmInstance(AUDIO_INSTANCE_1);
    ...
    if ((config->seqSourcePriority1 == audio_source_SPDIF) ||
        (config->seqSourcePriority2 == audio_source_SPDIF) ||
        (config->seqSourcePriority3 == audio_source_SPDIF) ||
        (config->seqSourcePriority4 == audio_source_SPDIF) ||
        (config->seqSourcePriority5 == audio_source_SPDIF) ||
        (config->seqSourcePriority6 == audio_source_SPDIF) ||
        (config->seqSourcePriority7 == audio_source_SPDIF))
    {
        if (config->UseAltPcm)
        {
            AudioConfigSetPcmInstance(AUDIO_INSTANCE_1);
