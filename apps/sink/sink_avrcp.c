...
{   
    ...
    if (getA2dpIndexFromAvrcp (avrcpIndex, (a2dp_link_priority*)&a2dpIndex) &&
            ...
            {        
               ...
               /* if the AG has previously set the stream state to 'suspended' when reaching the beginning or
                  ...
                  automatically do this for us */
                    if(sinkA2dpGetA2dpLinkSuspendState(a2dp_link) == a2dp_remote_suspended)
               {
                        a2dpSetSuspendState(a2dp_link, a2dp_not_suspended);
...
static void sinkAvrcpSetActiveConnectionFromIndex(uint16 new_index)
{
    uint16 current_index = AVRCP_DATA.active_avrcp;

    if (current_index != new_index)
    {
        /* Reset standard command queue. Vendor specific commands rely on
        memory allocated by the application and must be allowed to complete. */
        sinkAvrcpFlushAndResetStandardCommands(current_index);
...
DESCRIPTION
    Called when manually selecting A2DP1 or A2DP2 input, this function determines
            ...
            case audio_source_a2dp_2:
                {
                    ...
                    a2dp_link_priority index_for_source = (source == audio_source_a2dp_2 ? a2dp_secondary : a2dp_primary);

                    if((sinkAvrcpIsConnected(a2dp_primary))&&(BdaddrIsSame(getA2dpLinkBdAddr(index_for_source),
                                                                    &AVRCP_DATA.bd_addr[a2dp_primary])))
                    {
                        index = a2dp_primary;
                    }
                    else if((sinkAvrcpIsConnected(a2dp_secondary))&&(BdaddrIsSame(getA2dpLinkBdAddr(index_for_source),
                                                                    &AVRCP_DATA.bd_addr[a2dp_secondary])))
                    {
                    ...
                    if(index != a2dp_invalid)
                    {
                        ...
                        lUpdateMessage->bd_addr = getA2dpLinkBdAddrForIndex(index);
    ...
    {    
        ...
        if (sinkAvrcpIsInitialised())
        {
            if(getA2dpStatusFlag(CONNECTED, i) && (BdaddrIsSame(bd_addr, &AVRCP_DATA.bd_addr[i])) &&
               (sinkAvrcpGetPlayStatus(i) == (SinkAvrcp_play_status)play_status))
            {
                status = TRUE;
                break;
            }
