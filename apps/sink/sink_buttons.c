...
void ButtonsInit ( void )
{
    ...
    MessageChargerTask(&lButtonsTask->task);
...
void ButtonsInitHardware ( void )
{
#ifdef HYDRACORE
    {
        ...
        for (i = 0; i < BM_NUM_BUTTON_TRANSLATIONS; i++)
        {
            ...
            PioCommonAllBitsSetBit(&mask, pio);
        }

        PioCommonSetMap(&result, &mask, &mask);
        for (bank = 0; bank < NUMBER_OF_PIO_BANKS; bank++)
        {
            uint32 button_mask = mask.mask[bank];

            if (button_mask)
            {
                PioSetDir32Bank(bank, button_mask, 0);
                PioSet32Bank(bank, button_mask, 0);
                PioSetStrongBias32Bank(bank, button_mask, button_mask);

                PioDebounce32Bank(bank, button_mask, 4, 5);
            }
        }
    }
#endif /* HYDRACORE */
...
void ButtonsCheckForChangeAfterInit(void)
{
    ...
    PioCommonAllBitsInvert(PioCommonAllBitsRead(&pioState));
    PioCommonAllBitsSetFlags(&pioState,CHARGER_VREG_VALUE | CHARGER_CONNECT_VALUE);
    ...
    PioCommonAllBitsRead(&lButtonsTask->gOldPIOState);
    PioCommonAllBitsInvert(&lButtonsTask->gOldPIOState);
    PioCommonAllBitsSetFlags(&lButtonsTask->gOldPIOState,CHARGER_VREG_VALUE | CHARGER_CONNECT_VALUE);
    ButtonsUpdateButtonDebounceMask(&lButtonsTask->pio_debounce_mask);
 
    /* Debounce required PIO lines */
    if(!PioCommonDebounce(&lButtonsTask->pio_debounce_mask,
...
{   
    ...
    switch ( pId )
    {
        ...
        case MESSAGE_CHARGER_CHANGED:
        {
            ...
            PioCommonAllBitsInvert(PioCommonAllBitsRead(&pioState));
            PioCommonAllBitsSetFlags(&pioState,((uint32)m->vreg_en_high << VREG_PIN) | ((uint32)m->charger_connected << CHG_PIN));
        ...
        case MESSAGE_CHARGER_DETECTED:
        {
            ...
            UNUSED(m);
            B_DEBUG(("B: Rcv MESSAGE_CHARGER_DETECTED with Type %x, D+ %umV, D- %umV\n", m->attached_status, m->charger_dp_millivolts, m->charger_dm_millivolts));
            /* TODO: Add logic to handle type before setting D+/D-. */
            usbUpdateDataLines(m->attached_status, m->charger_dp_millivolts, m->charger_dm_millivolts);
        ...
        case MESSAGE_CHARGER_EMERGENCY:
        {
            const MessageEmergencyMode *m = (const MessageEmergencyMode *) (pMessage);
            UNUSED(m);
            B_DEBUG(("B: Rcv MESSAGE_CHARGER_EMERGENCY with emergency_mode %u\n", m->emergency_mode));
    ...
    for (i = 0; i < BM_NUM_BUTTON_TRANSLATIONS; i++)
    {
        ...
        if(lButtonsTask->pTranslations[i].input_number < physical_input_cap_0 )
        {
            /* if pio state is set then add to result */
             if(PioCommonAllBitsBitIsSet(PioState,lButtonsTask->pTranslations[i].input_number))
             {
        ...
        for (i = 0; i < BM_NUM_BUTTON_TRANSLATIONS; i++)
        {
            ...
            {          
                   ...
                   ((!include_capacitive_sensor) && (lButtonsTask->pTranslations[i].input_number < physical_input_cap_0)))
                {
                     ...
                     *  the initial release of the CSRA68100 ADK.
                     */
                    PioCommonAllBitsSetBit(mask,lButtonsTask->pTranslations[i].input_number);
                }
            }
        }  
