...
typedef struct __sink_power_global_data_t
{
    ...
    unsigned                unused2:1;
    unsigned                ext_fast:1;
    unsigned                ext_resistor_tolerance:3;
...
#include "sink_power_manager_charger_config_def.h"
static const sink_charge_setting default_charge_setting =
            {{FALSE, power_boost_disabled, vsel_chg, FALSE, FALSE, 0},
            {vterm_default, 0, 0}};
...
static void powerManagerUnstickCharger(void);
#ifdef ENABLE_PEER
...
static void  powerManagerGetMinandMaxBatteryLimit(sink_battery_limits *bLimits)
{
    ...
    sink_power_settings_config_def_t *ps_config = NULL;
    
    if (configManagerGetReadOnlyConfig(SINK_POWER_SETTINGS_CONFIG_BLK_ID, (const void **)&ps_config))
    {
#ifdef ENABLE_CHARGER_V2
        bLimits->max_battery_v = PowerChargerGetVtermMillivolts(GPOWER.default_vfloat);
#else
        ...
        configManagerReleaseConfig(SINK_POWER_SETTINGS_CONFIG_BLK_ID);
    }
}
...
static void powerManagerGetChargerSetting(uint16 level,sink_charge_setting *cSettings)
{
    ...
    if (configManagerGetReadOnlyConfig(CHARGER_CONTROL_CONFIG_BLK_ID, (const void **)&read_config))
    {
        if(cSettings)
        {
            cSettings->current.charge = read_config->chg_settings[level].charge;
#ifdef ENABLE_CHARGER_V2
            cSettings->current.boost = power_boost_disabled;
#else
            cSettings->current.boost = read_config->chg_settings[level].boost;
#endif
            cSettings->current.vsel = read_config->chg_settings[level].vsel;
            cSettings->current.power_off = read_config->chg_settings[level].power_off;
            cSettings->current.disable_leds = read_config->chg_settings[level].disable_leds;
            cSettings->current.current = read_config->chg_settings[level].current;
            cSettings->termination.type = read_config->chg_settings[level].termination_type;
            cSettings->termination.voltage = read_config->chg_settings[level].termination_voltage;
        }
...
static void powerManagerGetBatterySetting(uint16 level,sink_battery_setting *bSetting)
{
    sink_power_settings_config_def_t *read_config = NULL;
   
    /* Get the Config Data */
    if (configManagerGetReadOnlyConfig(SINK_POWER_SETTINGS_CONFIG_BLK_ID, (const void **)&read_config))
    {
        if(bSetting)
        {
...
RETURNS
    void
*/
static void updatePowerSettings(power_config* config, sink_power_settings_config_def_t* data)
{
    uint16 level = 0;

    if(config && data)
    {
        config->vref.adc.period_chg = D_SEC(data->config_p.p_config.vref.adc.period_chg);
        config->vref.adc.period_no_chg = D_SEC(data->config_p.p_config.vref.adc.period_no_chg);
        config->vref.adc.source = data->config_p.p_config.vref.adc.source;

        config->vbat.adc.period_chg = D_SEC(data->config_p.p_config.vbat.adc.period_chg);
        config->vbat.adc.period_no_chg = D_SEC(data->config_p.p_config.vbat.adc.period_no_chg);
        config->vbat.adc.source = data->config_p.p_config.vbat.adc.source;

        /* store the critical voltage as the min battery voltage */
        for (level = 0; level <= POWER_MAX_VBAT_LIMITS; level++)
        {
             config->vbat.limits[level].limit = data->bat_events[level].limit;
             config->vbat.limits[level].notify_period = data->bat_events[level].notify_period;
        }

        config->vthm.adc.period_chg = D_SEC(data->config_p.p_config.vthm.adc.period_chg);
        config->vthm.adc.period_no_chg = D_SEC(data->config_p.p_config.vthm.adc.period_no_chg);
        config->vthm.adc.source = data->config_p.p_config.vthm.adc.source;

        config->vthm.drive_ics = data->config_p.p_config.vthm.drive_ics;
        config->vthm.drive_pio = data->config_p.p_config.vthm.drive_pio;
        /* Can't drive PIO and internal current source! */
        PanicFalse(!(config->vthm.drive_ics && config->vthm.drive_pio));
        config->vthm.delay = data->config_p.p_config.vthm.delay;
        config->vthm.pio = data->config_p.p_config.vthm.pio;

        config->vthm.raw_limits = data->config_p.p_config.vthm.raw_limits;
        memcpy(&config->vthm.limits,&data->config_p.p_config.vthm.limits, POWER_MAX_VTHM_LIMITS*sizeof(config->vthm.limits[0]));

        config->vchg.adc.period_chg = 0;
        config->vchg.adc.period_no_chg = D_SEC(data->config_p.p_config.vchg.adc.period_no_chg);

        config->vchg.non_current_limit_period_chg = D_SEC(data->config_p.p_config.vchg.adc.period_chg);

        config->vchg.adc.source = data->config_p.p_config.vchg.adc.source;
    }
}
...
*/
#ifdef ENABLE_CHARGER_V2
static void updateChargerSafetySettings(power_config* config)
{
...
#else
#define updateChargerSafetySettings(config)
#endif
...
*/
#ifdef ENABLE_CHARGER_V2
static void updateChargerModeVoltageSelectorSettings(power_config* config)
{
...
static void updateChargerExternalResistorSettings(power_config* config)
{
    ...
    if(config)
    {
        ...
        if (size)
        {
            ...
            config->vchg.ext_resistor_tolerance = data->ext_resistor_tolerance;
            config->vchg.ext_resistor_value     = data->ext_resistor_value;
            config->vchg.quiescent_current      = data->quiescent_current;
...
static void updateChargerCurrentSettings(power_config* config)
{
    ...
    if(config)
    {
        ...
        if (size)
        {
            config->vchg.trickle_charge_ratio       = data->mode_currents.trickle_charge_ratio;
            config->vchg.precharge_ratio            = data->mode_currents.precharge_ratio;
...
static void updateChargerProgressSettings(power_config* config)
{
    ...
    if(config)
    {
        ...
        {
#ifdef ENABLE_CHARGER_V2
            GPOWER.default_vfloat                   = data->default_vfloat;
            config->vchg.standby_fast_hysteresis    = data->standby_fast_hysteresis;
            config->vchg.debounce_time              = data->debounce_time;
            config->vchg.iterm_ctrl                 = data->iterm_ctrl;
            config->vchg.pre_fast_threshold         = data->pre_fast_threshold;
...
/****************************************************************************
NAME
validateChargerCurrent
DESCRIPTION
    This function ensures that the charger current can not be configured
    higher than allowed for the charging mode.

RETURNS
    void
*/
static bool validateChargerCurrent(uint16 max_allowed, uint16 max_configured, uint8 ratio)
{
    uint32 current = ((max_configured * ratio) / ONE_HUNDRED_PERCENT);
    bool retval = (current <= max_allowed);
    if (!retval)
    {
        PM_DEBUG(("PM: Max configured %umA x Ratio %u%% = %lumA; Max allowed %umA\n", max_configured, ratio, current, max_allowed));
    }
    return retval;
}
...
{
    int i;
    uint16 max_configured_charge_current, max_charger_mode_current, resistance_milliohms;
    int num_regions;
...
#ifdef ENABLE_CHARGER_V2
    PanicFalse(config->vchg.precharge_ratio <= ONE_HUNDRED_PERCENT);
    PanicFalse(config->vchg.trickle_charge_ratio <= ONE_HUNDRED_PERCENT);
    PanicFalse(config->vchg.trickle_charge_ratio <= config->vchg.precharge_ratio);

    resistance_milliohms = PowerChargerExternalModeResistance(config->vchg.ext_resistor_value,
                                                              config->vchg.ext_resistor_tolerance);
    max_charger_mode_current = (MAX_SENSE_RESISTOR_MV * MV_IN_V) / resistance_milliohms;
    ...
    if (!validateChargerCurrent(max_charger_mode_current, max_configured_charge_current, FAST_CHARGE_RATIO))
    {
        PM_DEBUG(("PM: Charger Config for fast charge mode failed!\n"));
        Panic();
    }

    if (!validateChargerCurrent(MAX_INTERNAL_CURRENT, max_configured_charge_current, config->vchg.precharge_ratio))
    {
        PM_DEBUG(("PM: Charger Config for pre-charge mode failed!\n"));
        Panic();
    }

    if (!validateChargerCurrent(MAX_TRICKLE_CURRENT, max_configured_charge_current, config->vchg.trickle_charge_ratio))
    {
        PM_DEBUG(("PM: Charger Config for trickle charge mode failed!\n"));
        Panic();
    }
#else
    UNUSED(resistance_milliohms);
    max_charger_mode_current = MAX_INTERNAL_CURRENT;
    if (!validateChargerCurrent(max_charger_mode_current, max_configured_charge_current, FAST_CHARGE_RATIO))
    {
        PM_DEBUG(("PM: Charger Config failed!\n"));
        Panic();
    }
#endif
}
...
bool powerManagerInit(void)
{
    ...
    size = configManagerGetReadOnlyConfig(SINK_POWER_SETTINGS_CONFIG_BLK_ID, (const void **)&ps_config);
    ...
    updatePowerSettings(&config, ps_config);
    updateChargerProgressSettings(&config);
    updateChargerSafetySettings(&config);
    updateChargerCurrentSettings(&config);
    updateChargerExternalResistorSettings(&config);
    updateChargerModeVoltageSelectorSettings(&config);
...
bool powerManagerIsChargerConnected(void)
{
    ...
    PM_DEBUG(("PM: powerManagerIsChargerConnected(): ChargerStatus() returned %u\n", status));
...
bool powerManagerIsChargerFullCurrent(void)
{
    ...
    charger_status status = ChargerStatus();
    PM_DEBUG(("PM: powerManagerIsChargerFullCurrent(): ChargerStatus() returned %u\n", status));
#ifdef ENABLE_CHARGER_V2
...
static void powerManagerStartChargerStuckTimeout(uint16 timeout)
{
    if (timeout > 0)
    {
        ...
        MessageSendLater(&theSink.task, EventSysChargerStuckTimeout, NULL, D_MIN(timeout));
    }
}
...
static void powerManagerStopChargerStuckTimeout(void)
{
    ...
    MessageCancelAll(&theSink.task, EventSysChargerStuckTimeout);
}
...
{
    charger_status chg_status = ChargerStatus();
    PM_DEBUG(("PM: powerManagerChargerModeVoltageSelect(): ChargerStatus() returned %u\n", chg_status));
    return (chg_status == FAST_CHARGE ? GPOWER.vsel_fast_charge : vsel_chg);
}
#endif
...
bool powerManagerChargerSetup(voltage_reading* vthm)
{
    ...
    else
    {
        ...
        sink_charge_setting setting;
#ifdef ENABLE_USB
        sink_charge_current_t usb_limits;
        memset(&usb_limits,0,sizeof(sink_charge_current_t));
#endif
        memset(&setting,0,sizeof(sink_charge_setting));
        ...
        if(!powerManagerIsChargerConnected())
        {
            PM_DEBUG(("PM: Charger not connected\n"));
        ...
        if (powerManagerIsChargerStuck())
        {
            PM_DEBUG(("PM: Charger stuck, disabling charger\n"));
            setting.current.charge = FALSE;
        }
...
/****************************************************************************
NAME
    powerManagerPrepareForDormant

DESCRIPTION
      Prepare for dormant, return TRUE if okay

RETURNS
    bool
*/
bool powerManagerPrepareForDormant(void)
{
    if (powerManagerIsChargerConnected())
    {
        if (!PowerChargerEnabled())
        {
            ...
            PM_DEBUG(("PM: powerManagerPrepareForDormant(): ChargerStatus() returned %u\n", status));
            return (status == DISABLED_ERROR);
        }
        else
        {
            return FALSE;
...
/****************************************************************************
NAME
    powerManagerIsChargerStuck

DESCRIPTION
      Handle charger stuck timeout

RETURNS
    bool
*/
#ifdef ENABLE_CHARGER_V2
bool powerManagerIsChargerStuck(void)
{
    bool stuck;
    stuck = GPOWER.stuck;
    PM_DEBUG(("PM: Charger is %s\n", stuck ? "stuck" : "not stuck"));
    return stuck;
}
#endif
...
{
#ifdef ENABLE_CHARGER_V2
    PM_DEBUG(("PM: Unsticking charger\n"));
    GPOWER.stuck = FALSE;
    MessageCancelAll(&theSink.task, EventSysChargerStuckTimeout);
#endif
}
...
/****************************************************************************
NAME
    powerManagerChargerStuckTimeout
...
*/
#ifdef ENABLE_CHARGER_V2
void powerManagerChargerStuckTimeout(void)
{
    PM_DEBUG(("PM: Charger stuck timeout\n"));
    GPOWER.stuck = TRUE;
    powerManagerUpdateChargeCurrent();
}
#endif
    ...
    switch(powerManagerGetChargingDeficitAction())
    {
        ...
        case battery_low_output_action_limit_max_volume:
        {
              ...
              ))
            {
                ...
                modifyAndUpdateRoutedAudioVolume(same_volume,audio_output_group_main);
                   ...
                   )
            {
                ...
                if(GPOWER.low_batt_limit_wait_for_charger == FALSE)
                {
                    ...
                    /* force update of volume level to remove limit */
                    modifyAndUpdateRoutedAudioVolume(same_volume,audio_output_group_main);
                }
            }
        }
...
{
    sink_charging_deficit_limitations_config_def_t *data = NULL;
    uint16 size = 0;
     /* default to "Take no action" value */
    battery_low_output_action result = battery_low_output_action_none;

    size = configManagerGetReadOnlyConfig(SINK_CHARGING_DEFICIT_LIMITATIONS_CONFIG_BLK_ID, (const void **)&data);

    if (size)
    {
        result = (battery_low_output_action)data->action_when_battery_less_than_gas_gauge0;
        configManagerReleaseConfig(SINK_CHARGING_DEFICIT_LIMITATIONS_CONFIG_BLK_ID);
    }
