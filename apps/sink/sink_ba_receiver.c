...
static bool receiverCheckIfBroadcasterCanBeRemoved(bdaddr* addr)
{
    ...
    if(addr)
    {
        ...
        deviceManagerGetAttributes(&attributes, (const bdaddr *) addr);

        if(SINK_ATTRIBUTES_NOT_AVAILABLE(attributes) && GATT_ATTRIBUTES_NOT_AVAILABLE(attributes))
        {
...
void sinkReceiverStartAssociation(void)
{
    ...
    /* ensure we've stopped receiving */
...
bool baAudioSinkMatch(Sink sink)
{
    ...
    return sink == StreamSinkFromSource(BroadcastContextGetReceiverSource());
}
