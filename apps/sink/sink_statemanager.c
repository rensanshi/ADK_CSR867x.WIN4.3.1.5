...
static void stateManagerGoodbye(sinkGoodbyeState goodbye)
{
    ...
    if (goodbye == GOODBYE_STAY_AWAKE)
    {
        SM_DEBUG(("SM : Stay awake\n")) ;
    }
    else
    {
        ...
        {
            SM_DEBUG(("SM : Dormant\n")) ;
            sinkDormantEnable(powerManagerIsChargerConnected());
        }
        else
        {
...
/****************************************************************************
NAME
    stateManagerGoDormantWhenChargerDisabled

DESCRIPTION
    Go into dormant mode when the connected charger is disabled.

RETURNS
    void
*/
static void stateManagerGoDormantIfChargerDisabledOrDisconnected(void)
{
    stateManagerGoodbye(powerManagerPrepareForDormant() ? GOODBYE_DORMANT : GOODBYE_STAY_AWAKE);
}
...
DESCRIPTION
    Power off when the connected charger is disconnected.

RETURNS
    void
*/
static void stateManagerPowerOffIfChargerDisconnected(void)
{
    stateManagerGoodbye(powerManagerIsChargerConnected() ? GOODBYE_STAY_AWAKE : GOODBYE_POWER_OFF);
}
...
DESCRIPTION
    Attempt to power off or go into dormant mode depending charger state.
...
static void stateManagerAttemptToShutdown(void)
{
    ...
    {
        if (sinkDormantWakeUpFromNfc())
        {
            stateManagerGoDormantIfChargerDisabledOrDisconnected();
        }
        else
        {
            stateManagerPowerOffIfChargerDisconnected();
        }
    }
    else
    {
        stateManagerGoodbye(GOODBYE_STAY_AWAKE);
    }
}

/****************************************************************************
NAME
    stateManagerAttemptToPowerOffIfBatteryCritical

DESCRIPTION
    Power the device off if the battery voltage or temperature is critical and
...
RETURNS
    void
*/
static void stateManagerAttemptToPowerOffIfBatteryCritical(void)
{
