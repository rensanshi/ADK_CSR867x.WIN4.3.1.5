...
{       
    ...
    if(ind->status == hfp_disconnect_success || ind->status == hfp_disconnect_link_loss || ind->status == hfp_disconnect_abnormally)
    {
        ...
        /* update the AoV status */
        if (sinkHfpDataGetVoiceRecognitionSupported(PROFILE_INDEX(ind->priority)))
        {
            SLC_DEBUG(("SLC: Deactivate AoV\n"));
            sinkAovActivate(FALSE);
        }
        ...
        if((ind->status != hfp_disconnect_link_loss)&&(!(mask & conn_hfp)))
         sinkHfpDataSetProfileConnectedStatus(FALSE, PROFILE_INDEX(ind->priority));
        ...
        if(ind->priority == hfp_primary_link)
        {
            ...
            if(sinkHfpDataGetProfileStatusListId(PROFILE_INDEX(hfp_secondary_link)) != INVALID_LIST_ID)
            {
                ...
                sinkHfpDataSetProfileConnectedStatus(FALSE, hfp_secondary_link);
            }
        }
...
static void slcConnectionComplete(hfp_link_priority priority, Sink sink, const bdaddr* bd_addr)
{
    ...
    /* mark as connected */
    sinkHfpDataSetProfileConnectedStatus(TRUE,PROFILE_INDEX(priority));
...
bool sinkHandleSlcConnectCfm( const HFP_SLC_CONNECT_CFM_T *cfm )
{
    ...
    if (cfm->status == hfp_connect_success)
    {
        ...
        if (sinkHfpDataGetVoiceRecognitionSupported(PROFILE_INDEX(cfm->priority)))
        {
            SLC_DEBUG(("SLC: Activate AoV\n"));
            sinkAovActivate(TRUE);
        }
...
void slcContinueEstablishSLCRequest( void )
{
       ...
       (stateManagerGetState() != deviceLimbo))
    {
        ...
        else
        {
            ...
            else if(sinkDataEntrePairingModeOnConFailure() && !stateManagerIsConnected())
            {
