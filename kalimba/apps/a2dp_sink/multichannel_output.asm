...
.MODULE $M.multi_chan_output;
...
#else 
   DeclareCBuffer($multi_chan_sub_out_cbuffer_struc,$multi_chan_sub_out,1);
#endif 
   ...
   .VAR chain1_pcm_cbuffers_latency_measure[] =
       DYNAMIC, &$intermediate_fs_1, chain1_warp_ptr,  // cbuffer
       DYNAMIC, &$intermediate_fs_1, 0,                // port
       0;
   ...
   .VAR chain0_pcm_cbuffers_latency_measure[] =
       DYNAMIC, &$intermediate_fs_1, 0,           // cbuffer
       DYNAMIC, &$intermediate_fs_1, 0,           // port
       0;
...
#else
   TEMPLATE_SWITCH_ALT_OP(chain0_ch0_tone_switch_op, chain0_ch1_tone_switch_op, chain0_ch2_tone_switch_op, chain0_tone_mix_en, $MULTI_CHAN_CHAIN0_CHAN0_EN)
   //Check if Channel 1 is present. If present do dual mix, else mix only to channel 0
   TEMPLATE_SWITCH_ALT_OP(chain0_ch1_tone_switch_op, chain0_ch0_ch1_mix_op, chain0_ch0_mix_op, chain0_tone_mix_en, $MULTI_CHAN_CHAIN0_CHAN1_EN)
   TEMPLATE_MIX_DUAL_OUT_OP(chain0_ch0_ch1_mix_op, chain0_ch2_tone_switch_op, $MULTI_CHAN_CHAIN0_CHAN0, $MULTI_CHAN_CHAIN0_CHAN1, $tone0_in_left_resample_cbuffer_struc) 
   TEMPLATE_MIX_OP(chain0_ch0_mix_op, chain0_ch2_tone_switch_op, $MULTI_CHAN_CHAIN0_CHAN0, $tone0_in_left_resample_cbuffer_struc)
#endif
...
#else   
   TEMPLATE_SWITCH_ALT_OP(chain0_ch1_switch_op, chain0_ch1_mute_op, chain0_ch0_dither_switch_op, chain0_enables, $MULTI_CHAN_CHAIN0_CHAN1_EN)
#endif
...
#else
   TEMPLATE_SWITCH_ALT_OP(chain0_ch1_dc_remove_switch_op, chain0_ch0_dither_switch_op, chain0_ch1_dc_remove_op, dc_remove_disable, 1)
   TEMPLATE_DC_REMOVE_OP(chain0_ch1_dc_remove_op, chain0_ch0_dither_switch_op, 1, 1)
#endif   
...
.MODULE $M.multi_chan_build_channel_enable_mask;
   ...
   do channel_loop;
    ...
    r2 = r0 - $cbuffer.WRITE_PORT_OFFSET;
    r2 = r2 AND $cbuffer.TOTAL_PORT_NUMBER_MASK;
  
    Null = r2 - $cbuffer.NUM_PORTS;
    if POS jump not_enabled;
...
#ifndef DOWNMIX_STEREO_TONE                                                          // If not primary assume aux tone mixing
...
#else
      TEMPLATE_OP_CONFIG($M.multi_chan_output.chain0_ch0_ch1_mix_op, param, $cbops.auto_resample_mix.TONE_MIXING_RATIO_FIELD, r1)
      TEMPLATE_OP_CONFIG($M.multi_chan_output.chain0_ch0_mix_op, param, $cbops.auto_resample_mix.TONE_MIXING_RATIO_FIELD, r1)
#endif
...
.MODULE $M.multi_chan_tones_active;
   ...
   r0 = r0 OR r1;
#else
   TEMPLATE_OP_STATE($M.multi_chan_output.chain0_ch0_ch1_mix_op, param, $cbops.auto_resample_mix.INPUT_STATE_FIELD, r0)
   TEMPLATE_OP_STATE($M.multi_chan_output.chain0_ch0_mix_op, param, $cbops.auto_resample_mix.INPUT_STATE_FIELD, r1)
   r0 = r0 OR r1;
#endif
...
#if defined(SEC_CHANNEL_DISABLE) | defined(AUX_CHANNEL_DISABLE)
...
#if defined(SEC_CHANNEL_DISABLE)  
   r1 = $MULTI_CHAN_SECONDARY_CHANNELS_MASK;
#elif defined(AUX_CHANNEL_DISABLE)   
   r1 = $MULTI_CHAN_AUX_CHANNELS_MASK;
#elif defined(SEC_CHANNEL_DISABLE) & defined(AUX_CHANNEL_DISABLE)   
