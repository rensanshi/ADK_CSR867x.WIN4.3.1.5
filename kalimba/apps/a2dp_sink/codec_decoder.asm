...
/**********************************************/
// copy cbuffer_struc
// input: r3, source buffer struc
//        r4, dest buffer struc
// output: none
// trash:  r3;
/**********************************************/
.MODULE $M.copy_cbuffer_struc;
   .CODESEGMENT PM;
func:
    pushm <I0, I1>;
    push r10;
...
#define        MAX_MACRO(x,y)             ((x)>(y)?(x):(y))
...
#ifndef USB_AUDIOOUT_MEM_CONSOLIDATION   
...
#else   // USB_AUDIOOUT_MEM_CONSOLIDATION
...
#ifdef USB_AUDIOOUT_MEM_CONSOLIDATION   
   DeclareCBufferNoMem($audio_out_left_cbuffer_struc_template,$audio_out_left);
   DeclareCBufferNoMem($audio_out_right_cbuffer_struc_template,$audio_out_right);
   
   .VAR $usb_audio_out_left_cbuffer_struc_template[$cbuffer.STRUC_SIZE]  = USB_AUDIO_CBUFFER_SIZE,
                                                                            $M.tws.delay_buf_left,
                                                                            $M.tws.delay_buf_left;

   .VAR $usb_audio_out_right_cbuffer_struc_template[$cbuffer.STRUC_SIZE] =  USB_AUDIO_CBUFFER_SIZE,
                                                                            $M.tws.delay_buf_left + USB_AUDIO_CBUFFER_START_OFFSET_ROUNDUP_TO_2POWERN,
                                                                            $M.tws.delay_buf_left + USB_AUDIO_CBUFFER_START_OFFSET_ROUNDUP_TO_2POWERN;

#if defined(SPDIF_ENABLE) && defined(SPDIF_AAC_ENABLE)
   .VAR $usb_audio_out_lfe_cbuffer_struc_template[$cbuffer.STRUC_SIZE] =    USB_AUDIO_CBUFFER_SIZE,
                                                                            $M.tws.delay_buf_left + USB_AUDIO_CBUFFER_START_OFFSET_ROUNDUP_TO_2POWERN + USB_AUDIO_CBUFFER_START_OFFSET_ROUNDUP_TO_2POWERN,
                                                                            $M.tws.delay_buf_left + USB_AUDIO_CBUFFER_START_OFFSET_ROUNDUP_TO_2POWERN + USB_AUDIO_CBUFFER_START_OFFSET_ROUNDUP_TO_2POWERN;
#endif 
   ...
   DeclarePackedCBuffer($codec_in_cbuffer_struc,$codec_in, CODEC_CBUFFER_SIZE);
#else
...
#ifdef USE_PACKED_ENCODED_DATA
    // chains for copying from Port to packed buffer
   .VAR $pack_op_copy_struc[] =
      &pack_op,       // Pointer to struct describing a shift 16->24 bit conversion to apply when performing the copy
      1,                            // Number of inputs
      $CON_IN_PORT, // input buffer
      1,
      &$codec_in_cbuffer_struc;        // output
   
   .BLOCK pack_op;
      .VAR pack_op.next = $cbops.NO_MORE_OPERATORS;
      .VAR pack_op.func = &$cbops.pack_op;
      .VAR pack_op.params[$cbops.pack_op.STRUC_SIZE] =
         0,
         1,
      &$codec_in_cbuffer_struc;        // output
   .ENDBLOCK;
...
.MODULE $M.audio_out_copy_handler;
   ...
   if Z jump no_va_copy;

   r8 = $va_output_copy_struc;
   call $cbops.copy;
...
$read_bytes_from_port:
   ...
   //  become obsolute in packed_encoded_data mode  
   ...
   // get codec input port read pointer
   // already configured to read by byte
   r0 = M[r8 + $port_byte_read.INPUT_PORT_FIELD];
   call $cbuffer.get_read_address_and_size; // configure read port as 1 byte
   I1 = r0;     // change to I1 since I0 reserved for packed_encoded_buffer write
   L1 = r1;

    // r10 contains the amount of bytes
    // I0 port read address
    r0 = M[r8 + $port_byte_read.OUTPUT_BUFFER_FIELD];
    Null = r10 AND 1;   
    if Z jump skip_odd_byte;
    r1 = M[I1,0]; // port read single byte
    call $packed_cbuffer.put_word8; 
skip_odd_byte:
   
   // 16-bit words count
   r10 = r10 ASHIFT -1;
   if Z jump  skip_write_word;
   //   r0 = M[r8 + $port_byte_read.OUTPUT_BUFFER_FIELD];
   call $packed_cbuffer.put_word16_initialize;
   push r0;
   // M0 = 0, M1 = 1, r6 = 8, utility constant
   // I0/L0/B0, buffer write ptr 
   // I4/L4 write function table ptr,
   // r1, // input 16-bit word
   // r4, write function ptr
   // I1/L1 port address
   do  lp_write_word;
       r0 = M[I1, M0];
       r0 = r0 and 0xFF;  
       r0 = r0 LSHIFT r6, r1 = M[I1, M0];
       r1 = r1 and 0xFF;
       r0 = r0 or r1;
       call r5;  
lp_write_word:
...
.MODULE $M.con_in_copy_handler;
   ...
   Null = M[$M.voice_assistant.enabled];
   if Z jump no_va_copy;
   // VA copy port to cbuffer
   r8 = $va_input_copy_struc;
   call $cbops.copy;
no_va_copy:
#endif
   ...
   if Z jump no_odd_byte;
      ...
      r1 = r1 OR r0;
      r0 = $codec_in_cbuffer_struc;
      call $packed_cbuffer.put_word16;
#endif  // USE_PACKED_ENCODED_DATA     
...
$conn_init_cbuffers:
   ...
   r2 = r4;
   call $cbuffer.buffer_configure;
   // configure byte pointer 
#ifdef USE_PACKED_ENCODED_DATA
   r0 = $packed_cbuffer.INIT_BYTEPOS;
   M[$codec_in_cbuffer_struc + $packed_cbuffer.READ_BYTEPOS_FIELD] = r0;
   M[$codec_in_cbuffer_struc + $packed_cbuffer.WRITE_BYTEPOS_FIELD] = r0;
#endif    // USE_PACKED_ENCODED_DATA
...
#if defined(TWS_ENABLE) && defined(APTX_SINGLE_MODE_ENABLE)
  #ifdef USE_LEFT_ONLY
         r0 = $aptx.decode.left;
  #else
...
#if defined(TWS_ENABLE) && defined(APTX_SINGLE_MODE_ENABLE)
  #ifdef USE_LEFT_ONLY    // This only applies in TWS when slave is powered off.
         call $aptx.init_decoder.left;
  #else
    #ifdef USE_RIGHT_ONLY // This only applies in TWS when slave is powered off.
         call $aptx.init_decoder.right;
    #else
