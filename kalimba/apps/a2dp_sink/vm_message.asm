...
.MODULE $M.music_example_message;
   .DATASEGMENT DM;
   .CODESEGMENT MUSIC_EXAMPLE_MESSAGE_PM;
...
func:
   ...
   // Default condition is fail (0), only update if key passes and latch the 
   // result.
   Null = r1;
   if Z jump no_update_needed;
...
#if defined(WIRELESS_SUB_ENABLE) || defined(LATENCY_REPORTING) || defined(TWS_ENABLE) || defined(SPDIF_ENABLE)
/* Adjust latency data structure as per ANC mode. */
/* MMU buffers drains at ANC rate, so ANC affects latency as well. */
   r1 = &$M.main.cbuffers_latency_measure + 4*3 + 1;//mmu rate address to be updated for latency calculations
   r2 = &$intermediate_fs_1;
   r3 = &$inv_anc_fs;

   r0 = M[$ancMode];
   null = r0 - $ANC_NONE;
   if NZ jump check_anc_96k;
   M[r1] = r2; //MMU buffer will drain at dac rate.
   jump $pop_rLink_and_rts;

check_anc_96k:
   null = r0 - $ANC_96K;
   if NZ jump check_anc_192k;
   r0 = 96000;
   jump calc_inv_anc_fs;
...
calc_inv_anc_fs:
   ...
   M[r3] = r0;
   M[r1] = r3; //MMU buffer will drain at anc rate.

exit:
#endif // defined(WIRELESS_SUB_ENABLE) || defined(LATENCY_REPORTING) || defined(TWS_ENABLE) || defined(SPDIF_ENABLE)
