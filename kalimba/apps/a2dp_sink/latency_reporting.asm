...
$media_packet_boundary_detection:
   ...
   if Z jump done_with_latency_reporting;  // no room to store info
      ...
      if NEG r0 = r0 + r1;
#ifdef USE_PACKED_ENCODED_DATA 
      Null = r0 - 3; //In encoded space 2 physical memory address stores 3 data words. Check if we have more then 4.5 data words
#else
      Null = r0 - 4; //Check if we have more then 4 data words
#endif     
      if NEG jump done_with_latency_reporting; 
         ...
         if LS jump done_with_latency_reporting;
            ...
            I0 = r4;    //Load the start address of this copy
            L0 = r1; // added for the next dummy ptr move of cyclic buffer 
            r4 = M[I0, 1]; //dummy to move pointer
