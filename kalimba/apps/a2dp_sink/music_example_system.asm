...
//------------------------------------------------------------------------------
// on entry r7 = pointer to definition table
//          r8 = bypass flag
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
    ...
    if Z jump $pop_rLink_and_rts;                          // if Z there isn't a right channel
        m[r7 + $audio_proc.peq.PARAM_PTR_FIELD] = r8;
        call r3; // return from proc function
    jump $pop_rLink_and_rts;
...
// peq reset wrapper
//------------------------------------------------------------------------------
// -reset(zero) the filter
// on entry r7 = pointer to filter definition table
//------------------------------------------------------------------------------
zerodelay:
    r3 = m[r7 + $user_eq.EQ_ZERODELAY_FUNC];
...
#define MUSIC_MANAGER_CONFIG_FLAG_RAW                                   \
    ...
    + (flag_uses_DH_EQ_OPTION * uses_DH_EQ_OPTION) )
...
// *****************************************************************************
// MODULE:
//    $M.config_filter_defn_tables
//
// DESCRIPTION:
//    Initialize filter definition tables
//
// TRASH
//    Everything
//
// NOTE
//    Eqs in list below must run in the main process (not interrupt driven)
// *****************************************************************************
.MODULE $M.music_example.config_filter_defn_tabless;

   .CODESEGMENT MUSIC_EXAMPLE_PROCESS_PM;
   .DATASEGMENT DM;

.VAR $fdt_temp_config_bitmask = 0x005555;
.VAR/DMCONST eq_defntable_list[$music_example.FDT_LIST_COUNT] = 
#if uses_SPKR_EQ
...
#endif    
#if uses_BASS_BOOST
    $M.system_config.data.BoostEqDefnTable,
#else
    0,
#endif    
#if uses_ANC_EQ
    $M.system_config.data.AncEqDefnTable,
#else
    0,
#endif
#if uses_USER_EQ
    $M.system_config.data.UserEqDefnTable,
#else
    0,
#endif
#if uses_SPEAKER_CROSSOVER
    $M.system_config.data.spkr_ctrl_pri_eq_defn_table,
    $M.system_config.data.spkr_ctrl_sec_eq_defn_table,
#ifndef SUB_SUPPORT_DISABLE
    $M.system_config.data.spkr_ctrl_bass_eq_defn_table,
#else // SUB_SUPPORT_DISABLE
    0,
#endif // SUB_SUPPORT_DISABLE
#else  // uses_SPEAKER_CROSSOVER
    0,
    0,
    0,
#endif // uses_SPEAKER_CROSSOVER
#if uses_WIRED_SUB_EQ
    $M.system_config.data.WiredSubEqDefnTable;
#else
    0;
#endif


.VAR/DMCONST biquad_functables[] = 
    &sh_biquad_functable, // pointer to single precision history biquad functions
    &hq_biquad_functable, // pointer to HQ biquad functions
#if uses_DH_EQ_OPTION
    &dh_biquad_functable; // pointer to double precision history biquad functions
#else
    &hq_biquad_functable; // point to hq precision table if DH is disabled
#endif
.VAR/DMCONST sh_biquad_functable[] = 
    $audio_proc.peq.initialize,
    $audio_proc.peq.zero_delay_data,
    $audio_proc.peq.process,
    $audio_proc.peq.process_op;
.VAR/DMCONST hq_biquad_functable[] = 
    $audio_proc.hq_peq.initialize,
    $audio_proc.hq_peq.zero_delay_data,
    $audio_proc.hq_peq.process,
    $audio_proc.hq_peq.process_op;
#if uses_DH_EQ_OPTION
.VAR/DMCONST dh_biquad_functable[] = 
...
$config_filter_defn_tables:
   // configure biquad processor for each filter
   $push_rLink_macro;
   r4 = M[&$M.system_config.data.CurParams + $M.MUSIC_MANAGER.PARAMETERS.OFFSET_EQ_GLOBAL_CONFIG_WORD];
   r5 = 0;   // count
   fdt_config_loop:
      r0 = r4 AND 0x3; // r0 = biquad select (0=sh,1=hq,2=dh)
      // configure definition table with selected biquad function group
      r1 = M[r0 + &biquad_functables]; // r1=selected biquad table
      r2 = M[r5 + &eq_defntable_list]; // r2=defn table to configure
      if Z jump skip_defntable;
         // init fn
         r0 = M[r1 + $music_example.BIQUAD_FUNC_INIT];
         M[r2 + $user_eq.EQ_INIT_FUNC] = r0;
         r9 = r0;
         // zero fn
         r0 = M[r1 + $music_example.BIQUAD_FUNC_ZERODEL];
         M[r2 + $user_eq.EQ_ZERODELAY_FUNC] = r0;
         r10 = r0;
         // process fn
         r0 = M[r1 + $music_example.BIQUAD_FUNC_PROC];
#ifndef SUB_SUPPORT_DISABLE
         r3 = M[r1 + $music_example.BIQUAD_FUNC_PROC_OP];
         r1 = r2 - $M.system_config.data.spkr_ctrl_bass_eq_defn_table;
         if Z r0 = r3; // bass_eq_defn_table uses peq.process_op
#endif         
         M[r2 + $user_eq.EQ_PROC_FUNC] = r0;
         // call initialize and reset functions
         r7 = M[r2 + $user_eq.DEFINITION_TABLE_LEFT_DM_PTR];
         if Z jump bypass_left;
             pushm<r4,r5,r7,r9,r10>;
             call r9;   // call initialize function
             popm<r4,r5,r7,r9,r10>;
             pushm<r4,r5,r7,r9,r10>;
             call r10;  // call zero funtion
             popm<r4,r5,r7,r9,r10>;
         bypass_left:
         r7 = M[r2 + $user_eq.DEFINITION_TABLE_RIGHT_DM_PTR];
         if Z jump bypass_right;
             pushm<r4,r5,r7,r9,r10>;
             call r9;   // call initialize function
             popm<r4,r5,r7,r9,r10>;
             pushm<r4,r5,r7,r9,r10>;
         ...
         bypass_right:
      skip_defntable:
      r4 = r4 LSHIFT -$music_example.FDT_CONFIG_BITS;
      r5 = r5 + 1;
      Null = r5 - $music_example.FDT_LIST_COUNT;
      if NZ jump fdt_config_loop;
   jump $pop_rLink_and_rts;
.ENDMODULE;
