...
.MODULE $M.tws;
   ...
   DeclarePackedCBufferNoMem($codec_in_cbuffer_int_struc,$codec_in);
#else
   DeclareCBufferNoMem($codec_in_cbuffer_int_struc,$codec_in);
#endif      

#ifdef USE_PACKED_ENCODED_DATA
   DeclarePackedCBuffer($relay_cbuffer_struc,$relay_buffer, RELAY_CBUFFER_SIZE);
#else
   DeclareCBuffer($relay_cbuffer_struc,$relay_buffer, RELAY_CBUFFER_SIZE);
#endif
   ...
   .BLOCK $relay_copy_op;
      ...
      .VAR $relay_copy_op.func = &$cbops.unpack_op;
      .VAR $relay_copy_op.param[$cbops.unpack_op.STRUC_SIZE] =
         0,                          // Input index
         1,                          // Output index
         $relay_cbuffer_struc;
   .ENDBLOCK;
...
#else
   DeclareCBufferDM2(delay_buf_left_cbuffer_struc,delay_buf_left,($TWS_ALIGNMENT_DELAY_SIZE + 1));
...
#ifndef A2DP_SINK_LOWMEM //Right Delay not used for ROM ////Right Delay not used for ROM
...
// Update aptX codec mode depending on the routing mode
#ifdef APTX_SINGLE_MODE_ENABLE
      r0 = M[$app_config.io];
      // Check the codec type and update aptX mode if selected
      null = r0 - $APTX_IO;
      if Z call $tws.update_aptx_mode;
#endif
  
   // override Stereo Mode with DownMix
...
#ifndef A2DP_SINK_LOWMEM //Right Delay not used for ROM
   r8 = &$delay_right;
   call $tws.delay.initialize;
#endif 
   r1 = M[$tws.alignment_delay];
...
.MODULE $M.tws.wired_mode.enable;
   ...
   if NZ jump sra_conf_done1;
#ifdef  USB_AUDIOOUT_MEM_CONSOLIDATION   
      pushm <r3, r4>;   
      r3 = $audio_out_left_cbuffer_struc_template;
      ...
      call $M.copy_cbuffer_struc.func;
      popm <r3, r4>;
#endif // USB_AUDIOOUT_MEM_CONSOLIDATION
...
.MODULE $M.tws.wired_mode.disable;
   ...
   r0 = M[$app_config.io];
   Null = r0 - $USB_IO;   
   if  NZ  jump  reconfig_done;


      pushm <r3, r4>;   
      r3 = $usb_audio_out_left_cbuffer_struc_template;
      r4 = $audio_out_left_cbuffer_struc;
      call $M.copy_cbuffer_struc.func;
      r3 = $usb_audio_out_right_cbuffer_struc_template;
      r4 = $audio_out_right_cbuffer_struc;
      call $M.copy_cbuffer_struc.func;
      popm <r3, r4>;
...
// Update aptX codec mode depending on the project routing mode
#ifdef APTX_SINGLE_MODE_ENABLE
  #ifdef USE_LEFT_ONLY
         r2 = $TWS_ROUTING_LEFT;
  #else
    #ifdef USE_RIGHT_ONLY
         r2 = $TWS_ROUTING_RIGHT;
    #else
         r2 = $TWS_ROUTING_STEREO;
    #endif
  #endif
      M[$tws.routing_mode] = r2;
      r0 = M[$app_config.io];
      // Check the codec type and update aptX mode if selected
      null = r0 - $APTX_IO;
      if Z call $tws.update_aptx_mode;
...
#ifdef EFUSE
    pushm <r2, r3>;
    r2 = M[$M.KapLoader.FlashFileSystem.OTP_ram_data];
    r3 = EFUSE_TWS_MASK;
    Null = r1 and 0x2;
    if  NZ  jump  check_efuse;
    r3 = EFUSE_SHAREME_MASK;
    null = r1 - $SHAREME_MODE;
    if   Z  jump  check_efuse;
    jump   start_mode_change;
check_efuse:
    Null = r2 and r3;
    if  Z  jump  disable_mode_change;
    jump   start_mode_change; 
disable_mode_change:
    popm <r2, r3>;
    //  jump ack;  // could be an issue
    //  either interpreted as unsupported mode
    // $push_rLink_macro;
    // push r1;
    // jump  unsupported;
    //  or call $error
...
#ifdef LOWMEM_NO_RIGHT   
      r1 = &$M.system_config.data.stream_map_left_in;
      M[$left_to_right_obj + $M.audio_proc.stream_gain.OFFSET_OUTPUT_PTR] = r1;
#endif
...
#ifdef TWS_ENABLE
  #ifdef LOWMEM_NO_RIGHT   
      r1 = &$M.system_config.data.stream_map_right_in;
      M[$left_to_right_obj + $M.audio_proc.stream_gain.OFFSET_OUTPUT_PTR] = r1;
  #endif
...
#ifdef LOWMEM_NO_RIGHT  
   Null = M[$slave_flag];
   if Z jump bypass_slave_config; 
   // configure slave
   call $M.utils.config_tws;
   // keep slave flag
bypass_slave_config:   
#endif
...
#ifdef LOWMEM_NO_RIGHT  
   Null = M[$master_flag];
   if Z jump bypass_master_config; 
   // configure master
   call $M.utils.config_tws;
   // keep master flag
bypass_master_config:   
#endif
...
shareme:
#ifdef USE_PACKED_ENCODED_DATA
   call $relay.update_internal_ptr_packed;
#else
...
check_update_ptr:
   ...
   r5 = M[relay_temp];
#ifdef USE_PACKED_ENCODED_DATA
   call $relay.update_codec_ptr_packed;
#else
...
.MODULE $M.tws.extract_header;
   ...
   // if APTX and using packed cbuffer, we must update "relay getbitpos" with 
   // the packed cbuffer's bytepos
#if defined(APTX_ENABLE) && defined(USE_PACKED_ENCODED_DATA)
   r0 = M[r5 + $relay.INPUT_CBUFFER_FIELD];
...
continue1:
   ...
   readloop:
      ...
      if Z jump continue; // not APTx the continue
#ifdef USE_PACKED_ENCODED_DATA      
      r0 = M[r5 + $relay.CODEC_GET_BITPOS];  // Set bitpos to 16, so that next read from memory is 16 bits
      Null = M[r0];
      if NZ jump continue;
#endif
      r0 = M[I0,1];        //Dummy read to increment I0 SSSSSSTBD (why is this necissary?)
#ifdef  USE_PACKED_ENCODED_DATA
      r0 = 24;
#else
      r0 = 16;
...
buffer_underflow_occured:
   // restore pointers
   I0 = r7;
   // SGTBD: does I0 need to be written back to input cbuffer?
...
done_getting_header:
   ...
   r1 = I0;
#ifdef  USE_PACKED_ENCODED_DATA
   r2 = M[r5 + $relay.CODEC_GET_BITPOS];
   r2 = M[r2];
   r2 = r2 LSHIFT -3;
   r2 = r2 - 1;
...
#ifdef USE_PACKED_ENCODED_DATA
   #define APTX_BITPOS_START  24
#else //USE_PACKED_ENCODED_DATA
   #define APTX_BITPOS_START  16
#endif //USE_PACKED_ENCODED_DATA

.MODULE $M.aptxdec.getbits;
   .CODESEGMENT TWS_PM;
   .DATASEGMENT DM;
   .VAR $aptxdec.get_bitpos = APTX_BITPOS_START;
   .VAR/DM1 $aptxdec.bitmask_lookup[25] =
...
// ??? Tests C and/or V flag, but previous instruction will not modify those flags
   ...
   r2 = r2 - APTX_BITPOS_START;         // calc new shift amount
...
//*****************************************************************************
// MODULE:
//    $tws.update_aptx_mode
//
// DESCRIPTION:
//    For aptX in TWS we can use different codec modes depending on the TWS
//    configuration. We can decode a single channel to save power.
//
// INPUTS:
//    r2 - TWS Routing mode 
//
...
.MODULE $M.tws.update_aptx_mode;
   ...
   r2 = M[$tws.routing_mode];
      ...
      Null = r2 - $TWS_ROUTING_LEFT;
      if NZ jump check_right_aptx;
      
         // Update the decoder structure for APTX Single Channel - Left
         r0 = $aptx.decode.left;
         M[$decoder_codec_stream_struc + $codec.av_decode.ADDR_FIELD] = r0;
      ...
      check_right_aptx:
      null = r2 - $TWS_ROUTING_RIGHT;
      if NZ jump stereo_aptx;
    
         // Update the decoder structure for APTX Single Channel - Right
         r0 = $aptx.decode.right;
         M[$decoder_codec_stream_struc + $codec.av_decode.ADDR_FIELD] = r0;
         r0 = $aptx.reset_decoder;
         M[$decoder_codec_stream_struc + $codec.av_decode.RESET_ADDR_FIELD] = r0;
         r0 = $aptx.silence_decoder;
      ...
      stereo_aptx:
         ...
         // Update the decoder structure for APTX Stereo
         r0 = $aptx.decode;
         M[$decoder_codec_stream_struc + $codec.av_decode.ADDR_FIELD] = r0;
         r0 = $aptx.reset_decoder;
         M[$decoder_codec_stream_struc + $codec.av_decode.RESET_ADDR_FIELD] = r0;
