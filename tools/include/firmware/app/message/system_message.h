...
#include "app/validation/validation_if.h"
#include <source_.h>
#include <sink_.h>
...
/*!
  Report external serial flash partition erase result to VM application.
  The message content is a MessagePartitionEraseResult. This message is sent
  when partition erase is requested using StreamPartitionBackgroundErase().

 */
#define MESSAGE_PARTITION_ERASE_RESULT     (SYSTEM_MESSAGE_BASE_ + 63)
...
typedef struct
{
    validation_result result; /*!< validation result of an executable FS */
} MessageExeFsValidationResult;
...
/*!
  Message type received when a partition erase is completed
*/
typedef struct
{
    partition_erase_result result; /*!< result of partition erase */
    uint16 partition;            /*!< partition number */
} MessagePartitionEraseResult;
#endif
