...
/*!
    ...
    for more details on the difference between a dedicated charger and
    a standard host.

    Note that BlueCore will only report a correct AttachedStatus if
    PSKEY_USB_PIO_VBUS is set correctly and either a PIO or the internal
    battery charger (if available) is connected to the USB VBUS pin.

    @note
    When PSKEY_USB_CHARGER_DETECTION_ENABLEis set to Voltage source
    detection and this trap is invoked before the charger detection has
    completed, then BlueCore may return the status as DETACHED.
    However in such cases, once the charger detection process is completed,
    the VM application will be updated with the USB attach status.
*/
...
/*!
  ...
  @param string_index The USB String Descriptor number. Only index in the range 1 to 255
  can be used (0 is reserved for LANGID code).
  ...
  @note
  String indexes 1-4 are predefined and correspond to Manufacturer, Product, Serial Number
  and default Configuration String Descriptor respectively.
 
  @note
  The string_descriptor passed is in UTF16 format and MUST be NULL terminated.
  The NULL terminator will not be passed to USB host as part of the string
  descriptor when a string descriptor request is received, it is simply
  to denote the end of the string. Passing string_descriptor without a NULL
  terminator may lead to unexpected results even though this trap doesn't
  return FALSE.
  
  Example:
  Below code snippet registers string descriptor with index 11 as "Test".
