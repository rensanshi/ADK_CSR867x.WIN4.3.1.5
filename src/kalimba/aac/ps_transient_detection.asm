...
.MODULE $M.aacdec.ps_transient_detection;
   ...
   call $aacdec.initialise_ps_transient_detection_for_hybrid_freq_bins_flash;


   calculate_transient_attenuation_factors_outer_loop:


      M[$aacdec.tmp + $aacdec.PS_TRANSIENT_DETECTOR_HYBRID_MODE] = r0;

      // for freq_band={0:9(hybrid_freq_bands) || 10:21(remaining_qmf_freq_bands)}
      calculate_input_power_freq_bands_loop:


         //r0 = M[I1,1];//r0 = M[I6,1];     // parameter_band = map_freq_band_to_20_par_bands_table[freq_band]
         r1 = M[I5,1];  // start_k = frequency_border_table_20_par_bands[freq_band]
         ...
         r3 = M[$aacdec.tmp + $aacdec.PS_TRANSIENT_DETECTOR_HYBRID_MODE];
         ...
         r6 = r0 + M[$aacdec.tmp + $aacdec.PS_INPUT_POWER_MATRIX_BASE_ADDR];
         M[$aacdec.tmp + $aacdec.PS_INPUT_POWER_MATRIX_ROW_ADDRESS] = r6;
         ...
         Null = M[$aacdec.tmp + $aacdec.PS_TRANSIENT_DETECTOR_HYBRID_MODE];
         ...
         r0 = M[$aacdec.tmp + $aacdec.PS_TRANSIENT_DETECTOR_INTER_SUBBAND_STRIDE];

         r0 = r0 * r1 (int);
         r3 = r0 + M[$aacdec.tmp + $aacdec.PS_DECORRELATION_SK_IN_REAL_BASE_ADDR];
         r6 = r0 + M[$aacdec.tmp + $aacdec.PS_DECORRELATION_SK_IN_IMAG_BASE_ADDR];
         ...
         M[$aacdec.tmp + $aacdec.PS_TRANSIENT_DETECTOR_FREQ_BAND_START_FREQ] = r1;
         M[$aacdec.tmp + $aacdec.PS_TRANSIENT_DETECTOR_FREQ_BAND_END_FREQ] = r2;
         M[$aacdec.tmp + $aacdec.PS_TRANSIENT_DETECTOR_FREQ_BAND_WIDTH] = r2 - r1;
         ...
         calculate_input_power_time_samples_loop:
            ...
            if GT jump input_power_calculation_rmac_initialised;
               r6 = M[$aacdec.tmp + $aacdec.PS_INPUT_POWER_MATRIX_ROW_ADDRESS];
               r6 = r6 + M0;
               // rMAC(47:0) = [P[n][p_band]_MSW; P[n][p_band]_LSW]
               rMAC = M[r6];
               r0 = M[r6 + ($aacdec.PS_NUM_SAMPLES_PER_FRAME * $aacdec.PS_NUM_PAR_BANDS_IN_BASELINE_DECORRELATION)];
               rMAC0 = r0;
            input_power_calculation_rmac_initialised:

            // for k=start_k:end_k-1
            r0 = M[$aacdec.tmp + $aacdec.PS_TRANSIENT_DETECTOR_INTER_SUBBAND_STRIDE];
            r10 = M[$aacdec.tmp + $aacdec.PS_TRANSIENT_DETECTOR_FREQ_BAND_WIDTH];
            M3 = r0;
            ...
            if GT jump end_if_store_input_power_calculation;
               ...
               M[r6 + ($aacdec.PS_NUM_SAMPLES_PER_FRAME * $aacdec.PS_NUM_PAR_BANDS_IN_BASELINE_DECORRELATION)] = r0;
            ...
            #else
               r1 = r1 * ((1<<23) - $aacdec.PS_ALPHA_SMOOTH) (frac);
            #endif
            ...
            #else
               rMAC = rMAC + r5 * ((1<<23) - $aacdec.PS_ALPHA_SMOOTH);
            #endif
               if Z r1 = 0,
               ...
               if Z jump no_transient_detected;
                  ...
                  if GE jump no_transient_detected;
                     ...
                     Div = rMAC / r5;
                        ...
                        r1 = M[$aacdec.tmp + $aacdec.PS_TRANSIENT_DETECTOR_FREQ_BAND_START_FREQ];
                        r2 = M[$aacdec.tmp + $aacdec.PS_TRANSIENT_DETECTOR_FREQ_BAND_END_FREQ];
                        r0 = r0 * r2 (int);
                        r3 = I6 - r0;
                     ...
                     r4 = I4 - r0;
                     M[I7,1] = rMAC;   // store G_transient_ratio[n][p_band]
            ...
            if NZ jump end_if_crossing_qmf_thfgen_envelope;
               ...
               if NZ jump end_if_crossing_qmf_thfgen_envelope;
                  ...
                  r3 = (&$aacdec.sbr_x_real+640) + (($aacdec.SBR_tHFGen-$aacdec.SBR_tHFAdj)*$aacdec.X_SBR_WIDTH);
                  // r4 -> imag(X_SBR[ch=0][k=0][l=SBR_tHFAdj+6]))
                  r4 = (&$aacdec.sbr_x_imag+1664) + (($aacdec.SBR_tHFGen-$aacdec.SBR_tHFAdj)*$aacdec.X_SBR_WIDTH);
                  r5 = 0;
         ...
         if LE jump end_if_freq_band_gt_one_freq_band_loop;
            ...
            I0 = I0 + 2;   // I0 -> ps_power_peak_decay_nrg_prev[p_band+1]
            I3 = I3 + 2;   // I3 -> ps_power_smoothed_peak_decay_diff_nrg_prev[p_band+1]
            I2 = I2 + 2;   // I2 -> ps_smoothed_input_power_prev[p_band+1]
         end_if_freq_band_gt_one_freq_band_loop:


         M1 = M1 + 1;
         r0 = M[$aacdec.tmp + $aacdec.PS_TRANSIENT_DETECTOR_SUBBAND_LOOP_BOUND];
         Null = r0 - M1;
      if GT jump calculate_input_power_freq_bands_loop;


      call $aacdec.initialise_ps_transient_detection_for_qmf_freq_bins_flash;


      r0 = M[$aacdec.tmp + $aacdec.PS_TRANSIENT_DETECTOR_HYBRID_MODE];
      r0 = r0 - 1;
