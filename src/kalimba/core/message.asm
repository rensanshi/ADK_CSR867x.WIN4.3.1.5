...
.MODULE $M.message.received_service_routine;
   ...
   // skip message service routine if sequence number is the same as before
   r0 = M[$message.LAST_SEQ_FROM_MCU];
   r1 = M[$message.ACK_FROM_DSP];
   Null = r0 - r1;
   if EQ jump check_dmb_ports;
#endif
   ...
   no_handler:
      // If unknown ID either error or ignore the message
         ...
         M[$message.ACK_FROM_DSP] = r5;
#ifdef METADATA_SUPPORT
         // check DMB port for more metadata
         jump check_dmb_ports;
#else
         // pop rLink from stack
         jump $pop_rLink_and_rts;
#endif
...
check_dmb_ports:
   ...
   // check DMB read port for more metadata
   call $cbuffer.check_dmb_read_port;

   // update DMB write port for any port with pending meta-data
   r3 = M[$cbuffer.dmb_write_ports_mask];
...
// *****************************************************************************
// MODULE:
//    $message.send_ready_with_meta_wait_for_go
//
// DESCRIPTION:
//    Send the $MESSAGE_KALIMBA_READY_WITH_META message and wait for the
//    $MESSAGE_GO message
//
// INPUTS:
//    - r0 - metadata enable flags
//
// OUTPUTS:
//    - none
//
...
//
// *****************************************************************************
.MODULE $M.message.send_ready_with_meta_wait_for_go;
   .CODESEGMENT MESSAGE_SEND_READY_WITH_META_WAIT_FOR_GO_PM;
   .DATASEGMENT DM;
   ...
   // send message to request metadata and saying we're up and running!
   r2 = Null OR $MESSAGE_KALIMBA_READY_WITH_META;
   r3 = r4;
   call $message.send_short;
