...
{
    setMicrophoneInstanceGain(CSR_COMMON_EXAMPLE->voice_mic_params->mic_a, AUDIO_CHANNEL_A, CSR_COMMON_EXAMPLE->mic_gain_left);
    if(task->two_mic)
    {
        setMicrophoneInstanceGain(CSR_COMMON_EXAMPLE->voice_mic_params->mic_b, AUDIO_CHANNEL_B, CSR_COMMON_EXAMPLE->mic_gain_right);
    }
}
...
{
    Source mic_src = AudioPluginGetMicSource(CSR_COMMON_EXAMPLE->voice_mic_params->mic_a, AUDIO_CHANNEL_A);
    AudioPluginSetMicRate(mic_src, CSR_COMMON_EXAMPLE->voice_mic_params->mic_a.is_digital, mic_rate);

    if(task->two_mic)
    {
        Source mic_src2 = AudioPluginGetMicSource(CSR_COMMON_EXAMPLE->voice_mic_params->mic_b, AUDIO_CHANNEL_B);
        AudioPluginSetMicRate(mic_src2, CSR_COMMON_EXAMPLE->voice_mic_params->mic_b.is_digital, mic_rate);
...
static void disconnectMicrophones(const ExamplePluginTaskdata * const task)
{
    ...
    if(task->two_mic)
    {
        mic_src = AudioPluginGetMicSource(CSR_COMMON_EXAMPLE->voice_mic_params->mic_b, AUDIO_CHANNEL_B);
...
{
    AudioPluginSetMicPio(CSR_COMMON_EXAMPLE->voice_mic_params->mic_a, set);
    if(task->two_mic)
    {
        AudioPluginSetMicPio(CSR_COMMON_EXAMPLE->voice_mic_params->mic_b, set);
    }
}
...
{    
    ...
    KALIMBA_SEND_MESSAGE(MESSAGE_SETMODE, SYSMODE_PSTHRGH, 0, 0,
            (CSR_COMMON_EXAMPLE->voice_mic_params->mic_a.is_digital << 1) | CSR_COMMON_EXAMPLE->voice_mic_params->mic_b.is_digital);
}
