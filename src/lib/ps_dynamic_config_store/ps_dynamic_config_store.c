...
static dynamic_config_status_t getPsKeyUsedState(uint16 ps_key, bool *used);
...
int16 DynamicConfigGetConfigSize(const void *const_config_data, config_blk_id_t id)
{
    ...
    ps_key = resolvePSKey(const_config_data, id);
    ps_key_size = PsRetrieve(ps_key, NULL, 0);

    if(!ps_key_size)
    {
        bool used;

        if( (dynamic_config_success == getPsKeyUsedState(ps_key, &used)) && used )
        {
            /* 0 size block present in ps store. */
            return 0;
        }
        else
        {
...
static bool initUsedPsKeysPsKey(void)
{
    uint16 ps_key_size = PsRetrieve(psDynamicConfigStore->used_ps_keys_key, NULL, 0);
...
static dynamic_config_status_t setPsKeyUsedState(uint16 ps_key, bool used)
{
    ...
    retrieved_data_size = PsRetrieve(psDynamicConfigStore->used_ps_keys_key, psDynamicConfigStore->used_ps_keys, USED_PS_KEYS_SIZE);
    if(retrieved_data_size != USED_PS_KEYS_SIZE){
        return dynamic_config_error_retrieve;
    }
...
PARAMS
    ps_key PS key number.
    used   Pointer to boolean which keeps the used state of the given key.
RETURNS
    dynamic_config_success if everything was OK, error code otherwise.
*/
static dynamic_config_status_t getPsKeyUsedState(uint16 ps_key, bool *used)
...
dynamic_config_status_t DynamicConfigResetToDefaults(void)
{
    ...
    data_size = PsRetrieve(psDynamicConfigStore->used_ps_keys_key, psDynamicConfigStore->used_ps_keys, USED_PS_KEYS_SIZE);
