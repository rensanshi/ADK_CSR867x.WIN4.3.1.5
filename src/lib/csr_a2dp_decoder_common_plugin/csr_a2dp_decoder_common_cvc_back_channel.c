...
void CsrA2dpDecoderPluginDisconnectMic(A2dpPluginConnectParams *codecData)
{
    ...
    Source l_src = AudioPluginGetMicSource(codecData->mic_params->mic_a, AUDIO_CHANNEL_A);
    ...
    {
        Source r_src = AudioPluginGetMicSource(codecData->mic_params->mic_b, AUDIO_CHANNEL_B);
...
void csrA2dpDecoderPluginSetLowLatencyMode(AUDIO_MODE_T mode, const A2dpPluginModeParams *mode_params, A2DP_MUSIC_PROCESSING_T music_processing)
{
    ...
    KALIMBA_SEND_MESSAGE(LOW_LATENCY_SETMODE_MSG, sysmode, 0, call_state, (uint16)(( codecData->mic_params->mic_a.is_digital << 1 ) | codecData->mic_params->mic_b.is_digital) );
}
...
DESCRIPTION
    ...
    @return void
*/
void CvcMicSetGain(audio_mic_params audio_mic, audio_channel channel, T_mic_gain gain)
{
...
void CsrA2dpDecoderConnectBackChannel(A2dpPluginConnectParams * codecData, bool use_two_mic_back_channel)
{
    ...
    Source mic_source_a = AudioPluginGetMicSource(codecData->mic_params->mic_a, AUDIO_CHANNEL_A);
    /* set microphone sample rate */
    AudioPluginSetMicRate(mic_source_a, codecData->mic_params->mic_a.is_digital, BACK_CHANNEL_SAMPLE_RATE);
    /* Set mic bias pin */
    ...
    if(use_two_mic_back_channel)
    {
        ...
        Source mic_source_b = AudioPluginGetMicSource(codecData->mic_params->mic_b, AUDIO_CHANNEL_B);
        /* set mic b input sample rate */
        AudioPluginSetMicRate(mic_source_b, codecData->mic_params->mic_b.is_digital, BACK_CHANNEL_SAMPLE_RATE);
        /* set mic b bias pin */
                ...
                case LOW_LATENCY_READY_MSG:
                {
                    if (DECODER)
                    {
                        ...
                        /* update current dsp status */
                        {
                            ...
                            MAKE_AUDIO_MESSAGE(AUDIO_DSP_READY_FOR_DATA, dsp_ready_message);
                            ...
                            MessageSend(DECODER->app_task, AUDIO_DSP_READY_FOR_DATA, dsp_ready_message);
                ...
                case LOW_LATENCY_CODEC_MSG:
                {
                    if (DECODER)
                    {
                        ...
                        if(isCodecLowLatencyBackChannel())
                        {
                            ...
                            CvcMicSetGain(codecData->mic_params->mic_a, AUDIO_CHANNEL_A, lInput_gain_l);

                            if(DECODER->features.use_two_mic_back_channel)
                                CvcMicSetGain(codecData->mic_params->mic_b, AUDIO_CHANNEL_B, lInput_gain_r);
                        }
