...
const CvcPluginTaskdata csr_cvsd_cvc_2mic_headset_binaural_plugin = {{message_handler},cvc_2_mic_headset_binaural_nb, link_encoding_cvsd, 1, 0, BITFIELD_CAST(6, 0)};
const CvcPluginTaskdata csr_wbs_cvc_2mic_headset_binaural_plugin = {{message_handler},cvc_2_mic_headset_binaural_wb, link_encoding_msbc, 1, 1, BITFIELD_CAST(6, 0)};
...
static void message_handler ( Task task, MessageId id, Message message )
{
    ...
    else if(!csrCvcCommonDspHandleMixerMessage(task, id))
    {
...
static void handleAudioMessage ( Task task , MessageId id, Message message )
{
    ...
    switch (id)
    {
        ...
        {
			cvc_event_t event;
            event.cvc_disconnect_request.cvc_task = (CvcPluginTaskdata*)task;
            PRINT(("CVC: audio disconnect actioned\n"));			
			csrCvcCommonStateMachineHandleEvent(cvc_disconnect_request, &event);
		}
        break ;
