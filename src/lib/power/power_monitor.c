...
static void powerMonitorSetThermistorPio(bool drive)
{
    ...
    {
        PRINT(("POWER: Trp PioCommonSetPin(PIO = %u, pio_drive = %u, drive = %u) for Thermistor\n", power->config.vthm.pio, pio_drive, drive));
...
void powerMonitorReadAdc(Task task, MessageId id, uint32 period)
{
    ...
    case POWER_INTERNAL_VREF_REQ:
        PRINT(("POWER: Que POWER_INTERNAL_VREF_REQ in %ums\n", (int)period));
        break;

    case POWER_INTERNAL_VBAT_REQ:
        PRINT(("POWER: Que POWER_INTERNAL_VBAT_REQ in %ums\n", (int)period));
        break;

    case POWER_INTERNAL_VCHG_REQ:
        PRINT(("POWER: Que POWER_INTERNAL_VCHG_REQ in %ums\n", (int)period));
        break;

    case POWER_INTERNAL_VTHM_REQ:
        PRINT(("POWER: Que POWER_INTERNAL_VTHM_REQ in %ums\n", (int)period));
        break;

    case POWER_INTERNAL_VTHM_SETUP:
        PRINT(("POWER: Que POWER_INTERNAL_VTHM_SETUP in %ums\n", (int)period));
        break;

    case POWER_INTERNAL_CHG_PRG_MON_REQ:
        PRINT(("POWER: Que POWER_INTERNAL_CHG_PRG_MON_REQ in %ums\n", (int)period));
        break;

    default:
        PRINT(("POWER: Unexpected ADC request\n"));
        break;
    }
...
static void powerMonitorHandler(Task task, MessageId id, Message message)
{
    ...
    {
        case POWER_INTERNAL_VREF_REQ:
            PRINT(("POWER: Rcv POWER_INTERNAL_VREF_REQ\n"));
        ...
        case POWER_INTERNAL_VBAT_REQ:
            PRINT(("POWER: Rcv POWER_INTERNAL_VBAT_REQ\n"));
            /* Queue retry if ADC read fails */
        ...
        case POWER_INTERNAL_VCHG_REQ:
            PRINT(("POWER: Rcv POWER_INTERNAL_VCHG_REQ\n"));
            if(!powerMonitorReadAdcNow(task, id, &power->config.vchg.adc, power_init_vchg))
                powerMonitorReadAdc(task, id, 0);
        break;
        
        case POWER_INTERNAL_VTHM_SETUP:
            PRINT(("POWER: Rcv POWER_INTERNAL_VTHM_SETUP\n"));
        ...
        case POWER_INTERNAL_VTHM_REQ:
            PRINT(("POWER: Rcv POWER_INTERNAL_VTHM_REQ\n"));
        ...
        case POWER_INTERNAL_VBAT_NOTIFY_REQ:
            PRINT(("POWER: Rcv POWER_INTERNAL_VBAT_NOTIFY_REQ\n"));
        ...
        case POWER_INTERNAL_CHG_PRG_MON_REQ:
            PRINT(("POWER: Rcv POWER_INTERNAL_CHG_PRG_MON_REQ\n"));
        ...
        case MESSAGE_ADC_RESULT:
        {
            ...
            PRINT(("POWER: Rcv MESSAGE_ADC_RESULT with adc_source %u, reading %u\n", result->adc_source, result->reading));
