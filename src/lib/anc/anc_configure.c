...
static void closeMic(const audio_mic_params mic_params, audio_channel channel)
{
    Source anc_mic = AudioPluginGetMicSource(mic_params, channel);
...
static void closeMics(void)
{
    ...
    if (mic_params->enabled_mics & feed_forward_left)
    {
        closeMic(mic_params->feed_forward_left, AUDIO_CHANNEL_A);
    }
    if (mic_params->enabled_mics & feed_forward_right)
    {
        closeMic(mic_params->feed_forward_right, AUDIO_CHANNEL_B);
    }
}
