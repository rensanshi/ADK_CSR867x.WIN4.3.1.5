...
void CsrAgAudioPluginConnect( CsrAgAudioPluginTaskData *task, Sink audio_sink , AUDIO_SINK_T sink_type, uint16 volume , uint32 rate , bool stereo , AUDIO_MODE_T mode , const void * params , Task app_task)
{
   ...
   CSR_AG_AUDIO->voice_mic_params = (const voice_mic_params_t *) audio_params->mic;
   ...
   if (CSR_AG_AUDIO->voice_mic_params)
   {
       /* Set mic pin */
       AudioPluginSetMicPio(CSR_AG_AUDIO->voice_mic_params->mic_a, TRUE);
       if(task->two_mic)
           AudioPluginSetMicPio(CSR_AG_AUDIO->voice_mic_params->mic_b, TRUE);
   }
...
void CsrAgAudioPluginDisconnect( CsrAgAudioPluginTaskData *task )
{
    ...
    if (CSR_AG_AUDIO->voice_mic_params)
    {
        mic_src_a = AudioPluginGetMicSource(CSR_AG_AUDIO->voice_mic_params->mic_a, AUDIO_CHANNEL_A);
        mic_src_b = AudioPluginGetMicSource(CSR_AG_AUDIO->voice_mic_params->mic_b, AUDIO_CHANNEL_B);
        ...
        AudioPluginSetMicPio(CSR_AG_AUDIO->voice_mic_params->mic_a, FALSE);
        if(task->two_mic)
            AudioPluginSetMicPio(CSR_AG_AUDIO->voice_mic_params->mic_b, FALSE);
    }
...
static void ConnectAudio (CsrAgAudioPluginTaskData *task, bool stereo )
{
    ...
    if ( CSR_AG_AUDIO->audio_sink )
    {
        ...
        if (CSR_AG_AUDIO->voice_mic_params)
        {
            ...
            mic_src = AudioPluginGetMicSource(CSR_AG_AUDIO->voice_mic_params->mic_a, AUDIO_CHANNEL_A);
            AudioPluginSetMicRate(mic_src, CSR_AG_AUDIO->voice_mic_params->mic_a.is_digital, CSR_AG_AUDIO->dac_rate);
            ...
            if( task->two_mic )
            {
                ...
                mic_src2 = AudioPluginGetMicSource(CSR_AG_AUDIO->voice_mic_params->mic_b, AUDIO_CHANNEL_B);
                AudioPluginSetMicRate(mic_src2, CSR_AG_AUDIO->voice_mic_params->mic_b.is_digital, CSR_AG_AUDIO->dac_rate);
...
DESCRIPTION
    Set mic gain
*/
static void CsrAgAudioMicSetGain(audio_mic_params audio_mic, audio_channel channel, T_mic_gain gain)
{
...
static void CodecMessage (CsrAgAudioPluginTaskData *task, T_mic_gain input_gain_l, T_mic_gain input_gain_r, uint16 output_gain )
{
    ...
    if(CSR_AG_AUDIO)
    {
        ...
        {
            /* Set input gain(s) */
            CsrAgAudioMicSetGain(CSR_AG_AUDIO->voice_mic_params->mic_a, AUDIO_CHANNEL_A, input_gain_l);
            if( task->two_mic )
                CsrAgAudioMicSetGain(CSR_AG_AUDIO->voice_mic_params->mic_b, AUDIO_CHANNEL_B, input_gain_r);
        }
    }
}
                ...
                case KALIMBA_MSG_WARP_VALUES:
                    {
                        if (CSR_AG_AUDIO)
                        {
                            MAKE_AUDIO_MESSAGE_WITH_LEN(AUDIO_DSP_IND, CSR_AG_AUDIO_WARP_NUMBER_VALUES, message);
                            ...
                            message->value[2] = m->c;
                            message->value[3] = m->d;
                            MessageSend(CSR_AG_AUDIO->app_task, AUDIO_DSP_IND, message);
                        }
                    }
                    break;
