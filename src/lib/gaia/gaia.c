...
#define GAIA_UPGRADE_HOST_DATA_REQ (0x04)
...
DESCRIPTION
    Sets partition block size in upgrade and gaia libraries
*/
static void set_partition_block_size(uint32 size)
{
    UpgradeSetPartitionDataBlockSize(size);
    gaia->upgrade_buffer_size = size;
}
...
static bool set_data_endpoint_mode(gaia_transport *transport, uint8 mode)
{
    switch(mode)
    {
        case GAIA_DATA_ENDPOINT_MODE_NONE:
            ...
            set_partition_block_size(UPGRADE_MAX_PARTITION_DATA_BLOCK_SIZE);
            return TRUE;
        ...
        case GAIA_DATA_ENDPOINT_MODE_RWCP:
            ...
            set_partition_block_size(GAIA_RWCP_UPGRADE_BUFFER_SIZE);
            return TRUE;
...
{
     uint8* buffer  = PanicUnlessMalloc(gaia->upgrade_buffer_size + GAIA_UPGRADE_HEADER_SIZE);
     uint8 count = 0;
...
static void getNextUpgradeData(gaia_transport *transport)
{
     ...
     upgrade_len = MIN(available_bytes, gaia->upgrade_buffer_size);
...
{
    if(transport->type != gaia_transport_spp)
        return FALSE;

    if(payload_length <= (gaia->upgrade_buffer_size + GAIA_UPGRADE_HEADER_SIZE))
        return FALSE;
...
static void upgrade_control(gaia_transport *transport, uint8 *payload, uint8 payload_length)
{
    if (transport == gaia->upgrade_transport)
    {
        if(isLargeDataUpgrade(transport, payload, payload_length))
        {
            ...
            SourceDrop(gaiaTransportGetSource(transport), GAIA_OFFS_PAYLOAD + GAIA_UPGRADE_HEADER_SIZE);
 ...
 */
static void handle_upgrade_transport_data_cfm(uint8* data, uint16 size_data, upgrade_status_t status)
{
 ...
 */
static void handle_upgrade_transport_data_ind(uint8* data, uint16 size_data)
{
    ...
    if (isRwcpEnabled())
    {
        ...
        gaia->upgrade_buffer_size = GAIA_RWCP_UPGRADE_BUFFER_SIZE;
    }
    else
    {
        gaia->upgrade_buffer_size = UPGRADE_MAX_PARTITION_DATA_BLOCK_SIZE;
    }

    if(UpgradeIsPartitionDataState() && data[0] == GAIA_UPGRADE_DATA_BYTES_REQ)
    {
        if((first == FALSE) && (size_data > GAIA_UPGRADE_DATA_BYTES_REQ_OFFSET))
        {
            if(ByteUtilsGet4BytesFromStream(&data[GAIA_UPGRADE_DATA_SIZE_OFFSET]) == gaia->upgrade_buffer_size)
            {
                uint32 size = 0;
...
*/
    ...
    case UPGRADE_TRANSPORT_DATA_IND:
        ...
        if (gaia->upgrade_transport && gaia->upgrade_transport->notify_vmup)
        {
            ...
            handle_upgrade_transport_data_ind(ind->data, ind->size_data);
        }
        break;
