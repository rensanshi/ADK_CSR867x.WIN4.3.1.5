...
{
    uint16 interval;
    /* VK15 TODO this is non plugin specific and makes sense to move to broadcast_cmd or any other new library.*/
...
/***************************************************************************
NAME
    baCsbDecoderShutdownAudio
 
DESCRIPTION
    Utility function to Disconnect DSP from DACs
...
RETURNS
    void
*/
static void baCsbDecoderShutdownAudio(void)
{
    /* Disconnect DSP from DACs/I2S */
...
void CsrBaReceiverAudioPluginDisconnect(const A2dpPluginTaskdata * const task)
{
    ...
    SetAudioInUse(FALSE);
    SetAudioBusy(NULL);
    /* VK15 TODO : New Library API to handle this stuff
    Reset SCM receiver as there may be partial messages */
        ...
        case MESSAGE_FROM_KALIMBA:
        {
            ...
            switch (kal_msg->id)
            {
                ...
                case KALIMBA_MSG_CSB_SAMPLE_RATE_CHANGED:
                    ...
                    baCsbDecoderShutdownAudio();
                    baCsbDecoderSetupAudio((uint32)((kal_msg->data[0] << 8) + kal_msg->data[1]));
                    break;
